...
window.deleteAdminMessage=async function(docId){if(!confirm("حذف الرسالة نهائياً؟"))return;try{await db.collection('contact_messages').doc(docId).delete();showToast("تم حذف الرسالة.");loadAdminMessages()}catch(e){showToast("فشل الحذف!")}};let isApprovingWorkout=!1;async 
...
async function approveWorkout(docId) {
    ...
    try {
        ...
        if (dethronedVictim) {
            ...
            const selfMsg = currentLang === 'en' ? `👑 You crushed a record in ${userData.city}! You took down a map legend with ${maxW}kg!` : `👑 لقد حطمت عرشاً في ${userData.city}! أنت الآن من أساطير الخريطة بوزن ${maxW}kg!`;

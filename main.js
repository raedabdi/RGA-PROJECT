...
window.deleteFriendFromProfile = async function(friendId) {
    ...
    if (confirm(confirmMsg)) {
        try {
            // 1. تحديث الـ LocalStorage فوراً (عشان الزر يتغير قدامك بلحظتها)
            let localData = JSON.parse(localStorage.getItem('currentUser') || '{}');
            ...
            // 2. إعادة رسم البروفايل فوراً
            viewPlayerProfile(friendId);

            // 3. التحديث في قاعدة البيانات (حذف متبادل)
            ...
            if (myDoc.exists) {
                ...
                await myRef.update({ myFriendsList: myFriends.filter(f => f.id !== friendId) });
            }

            const friendDoc = await friendRef.get();
            if (friendDoc.exists) {
                let theirFriends = friendDoc.data().myFriendsList || [];
                await friendRef.update({ myFriendsList: theirFriends.filter(f => f.id !== currentUser.uid) });
            }

...
// الكود الجديد والمطور لحفظ صورة البروفايل (نظام Base64 لمنع الرمشة نهائياً)
...
if (cropSaveBtn) {
    cropSaveBtn.onclick = async () => {
        ...
        try {
            // السحر هنا: تحويل الصورة لـ Base64 بحجم (200x200) وجودة 60% عشان يكون حجم النص خفيف جداً على الفايربيس
            const base64Image = cropper.getCroppedCanvas({ width: 200, height: 200 }).toDataURL('image/jpeg', 0.6);

            const user = auth.currentUser;
            
            // حفظ النص مباشرة في الداتا بيس (تخطينا الـ Storage بالكامل)
            await db.collection('users').doc(user.uid).update({ photoURL: base64Image });
            
            // تحديث واجهة المستخدم والـ LocalStorage فوراً
            const saved = JSON.parse(localStorage.getItem('currentUser') || '{}');
            saved.photoURL = base64Image;
            localStorage.setItem('currentUser', JSON.stringify(saved));
            
            renderUI(saved); // استدعاء دالة الرسم لتحديث الداشبورد

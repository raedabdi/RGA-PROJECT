    ...
    try {
        ...
        if (myUid && myUid !== targetUid) {
            ...
            if (isFriend) {
                ...
                // تم تعديل التنسيق هنا لضمان المحاذاة التامة ومنع انكسار الحاوية
                friendActionHTML = `
                    <div style="display: flex; gap: 10px; margin-top: 15px; width: 100%; align-items: center;">
                        <button class="btn-primary" style="flex: 1; height: 50px; background: rgba(0, 242, 167, 0.1); border-color: #00f2a7; color: #00f2a7; cursor: default; display: flex; align-items: center; justify-content: center; gap: 8px; padding: 0;">
                            ${friendBtnText}
                        </button>
                        <div style="position: relative; height: 50px; display: flex; align-items: center;">
                            ...
                            <div id="profile-friend-menu" class="member-action-menu-content" style="position: absolute; top: 55px; left: 0; background: rgba(10, 20, 41, 0.98); border: 1px solid rgba(255, 77, 77, 0.3); border-radius: 12px; padding: 5px; box-shadow: 0px 10px 30px rgba(0,0,0,0.8); min-width: 150px; z-index: 1000;">
                                <div class="dropdown-item-pro danger" onclick="deleteFriendFromProfile('${targetUid}')" style="padding: 12px; font-size: 0.9rem; display: flex; align-items: center; justify-content: center; gap: 10px; color: #ff4d4d; cursor: pointer; border-radius: 8px; white-space: nowrap;">
                                    ${unfriendBtnText}
                                </div>
                            </div>
                        </div>
                ...
                if (reqDoc.exists) {
                    ...
                    friendActionHTML = `
                        <button class="btn-primary" style="width: 100%; height: 50px; margin-top: 15px; background: rgba(255,255,255,0.05); border-color: var(--slate); color: var(--slate); cursor: not-allowed; display: flex; align-items: center; justify-content: center; gap: 8px;" disabled>
                            ${reqSentText}
                        </button>
                    `;
                } else {
                    ...
                    friendActionHTML = `
                        <button class="btn-primary" id="profile-add-friend-btn" style="width: 100%; height: 50px; margin-top: 15px; box-shadow: 0 0 15px rgba(0,242,167,0.2); display: flex; align-items: center; justify-content: center; gap: 8px;" onclick="sendFriendRequest('${targetUid}')">
                            ${addFriendText}
                        </button>
                    `;
                }
        ...
        if (data.clanId) {
            ...
            clanInfoHTML = `
                <div class="premium-clan-badge" onclick="closeProfileAndOpenClan('${data.clanId}')" style="margin-top:15px;">
            ...
            // كود زر الدعوة (تم التعديل لإضافة فحص الدعوة المعلقة)
            const txtInvite = lang === 'en' ? 'Invite to Clan' : 'دعوة للعصابة';
            const inviteId = `invite_${myUid}_${targetUid}`;
            
            clanInfoHTML = `<button id="clan-invite-btn" class="btn-primary" style="width:100%; height: 50px; margin-top:15px; background: rgba(0, 242, 167, 0.1); border: 1px solid var(--primary-color); color:var(--primary-color); display: flex; align-items: center; justify-content: center; gap: 8px;" onclick="sendClanInvite('${targetUid}')">
                <i class="fa-solid fa-envelope-open-text"></i> ${txtInvite}
            </button>`;

            // فحص سريع في الخلفية إذا كانت الدعوة مرسلة اليوم لقفل الزر
        ...
        modal.innerHTML = `
            ...
            <div style="padding: 20px; max-width: 600px; margin: 0 auto; width: 100%; padding-bottom: 50px;">
                ...
                <div class="earned-badges-showcase">${earnedBadgesHTML}</div>
            </div>
        `;
...
window.sendFriendRequest = async function(targetUid) {
    ...
    if (myFriends.find(f => f.id === targetUid)) {
        showToast(lang === 'en' ? "Already friends!" : "أنتم أصدقاء بالفعل!");
        return;
    }

    // تحديد الزر الذي تم الضغط عليه (سواء في البحث أو في البروفايل)
    const profileBtn = document.getElementById('profile-add-friend-btn');
    const searchResultCont = document.getElementById('search-result-container');
    const searchBtn = searchResultCont ? searchResultCont.querySelector('button') : null;

    try {
        const myName = (myData.firstName + " " + (myData.lastName || "")).trim();
        const myPhoto = myData.photoURL || "https://i.ibb.co/9mPmHXkh/cropped-circle-image-2.png";
        const requestDocId = 'freq_' + currentUser.uid;
        
        // إرسال الطلب للفايربيس
        ...
        // 🔥 التحديث اللحظي للواجهة (بدون ريفرش)
        const updateButtonUI = (btn) => {
            if (btn) {
                ...
                btn.innerHTML = lang === 'en' 
                    ? '<i class="fa-solid fa-clock-rotate-left"></i> Request Sent ⏳' 
                    : 'تم الإرسال ⏳ <i class="fa-solid fa-clock-rotate-left"></i>';
            }
        };

        // تحديث الزر في مودال البروفايل إذا كان مفتوحاً
        updateButtonUI(profileBtn);
        
        // تحديث الزر في نتائج البحث إذا كان ظاهراً
        if (searchBtn) updateButtonUI(searchBtn);
...
<div class="member-actions-menu">
    <button class="dots-btn" onclick="toggleMemberMenu('menu-${m.uid}', this)"><i class="fa-solid fa-ellipsis-vertical"></i></button>
    <div id="menu-${m.uid}" class="dropdown-content-pro" style="display: none; border-radius: 8px; box-shadow: 0px 5px 15px rgba(0,0,0,0.5);">

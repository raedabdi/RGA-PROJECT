...
modal.style.backdropFilter = 'blur(10px)';
    ...
    try {
        ...
        // --- تجهيز حالة العصابة (Clan) وزر الدعوة ---
        ...
        if (data.clanId) {
            ...
            // تصميم الزر الفخم (VIP) مع الحفاظ على ID الكلان عشان نحدثه فوراً
            clanInfoHTML = `
                <div class="premium-clan-badge" onclick="closeProfileAndOpenClan('${data.clanId}')">
            ...
            // جلب اسم العصابة من الفايربيس ووضعه داخل الزر مباشرة
            db.collection('clans').doc(data.clanId).get().then(doc => {
                const nameEl = document.getElementById('dynamic-clan-name');
                if (doc.exists && nameEl) {
                    nameEl.innerText = doc.data().name; // وضع الاسم الحقيقي
                } else if (nameEl) {
                    nameEl.innerText = lang === 'en' ? 'Unknown Clan' : 'عصابة غير معروفة';
                }
            }).catch(e => console.log(e));
        }

        // 2. إذا لم يكن في كلان، وأنا أمتلك كلان، أظهر زر "دعوة"
        else if (myUserData.clanId) {
            const txtInvite = lang === 'en' ? 'Invite to Clan' : 'دعوة للعصابة';
            
            clanInfoHTML = `
                <button class="btn-primary" style="width:100%; margin-top:15px; background: rgba(0, 242, 167, 0.1); border: 1px solid var(--primary-color); color:var(--primary-color);" 

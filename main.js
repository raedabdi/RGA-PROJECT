...
async function viewPlayerProfile(targetUid) {
    ...
    try {
        ...
        // --- 🤝 إضافة نظام طلبات الصداقة داخل البروفايل ---
        ...
        if (myUid && myUid !== targetUid) {
           ...
           if (isFriend) {
                // حالة 3: أصدقاء (مع زر الحذف بتصميم مضغوط وأنيق)
                friendActionHTML = `
                    <div style="display: flex; gap: 10px; margin-top: 15px; width: 100%;">
                        <button class="btn-primary" style="flex: 1; background: rgba(0, 242, 167, 0.1); border-color: #00f2a7; color: #00f2a7; cursor: default;">
                            <i class="fa-solid fa-user-check"></i> ${lang === 'en' ? 'Friend 🤝' : 'صديق 🤝'}
                        </button>
                        <div class="member-actions-menu" style="position: relative; display: flex;">
                            <button class="btn-primary dots-btn" onclick="toggleMemberMenu('profile-friend-menu', this)" style="padding: 0; width: 45px; height: 100%; background: rgba(255,255,255,0.05); border-color: var(--slate); color: var(--slate); display:flex; align-items:center; justify-content:center;">
                                <i class="fa-solid fa-ellipsis-vertical"></i>
                            </button>
                            
                            <div id="profile-friend-menu" class="dropdown-content-pro" style="display: none; background: rgba(10, 20, 41, 0.95); border: 1px solid rgba(255, 77, 77, 0.3); border-radius: 8px; padding: 4px; box-shadow: 0px 5px 20px rgba(0,0,0,0.8); min-width: 140px; z-index: 99999;">
                                <div class="dropdown-item-pro danger" onclick="deleteFriendFromProfile('${targetUid}')" style="padding: 10px; font-size: 0.9rem; display: flex; align-items: center; gap: 10px; color: #ff4d4d; cursor: pointer; border-radius: 5px; white-space: nowrap; margin: 0;">
                                    <i class="fa-solid fa-user-minus"></i> ${lang === 'en' ? 'Unfriend' : 'إزالة الصديق'}
                                </div>
                            </div>
                ...
                // فحص إذا كان الطلب قيد الانتظار مسبقاً
                const reqDoc = await db.collection('users').doc(targetUid).collection('notifications').doc('freq_' + myUid).get();
                if (reqDoc.exists) {
                    // حالة 2: تم إرسال الطلب
                    friendActionHTML = `
                        <button class="btn-primary" style="width: 100%; margin-top: 15px; background: rgba(255,255,255,0.05); border-color: var(--slate); color: var(--slate); cursor: not-allowed;" disabled>
                            <i class="fa-solid fa-clock-rotate-left"></i> ${lang === 'en' ? 'Request Sent ⏳' : 'تم الإرسال ⏳'}
                        </button>
                    `;
                } else {
                    // حالة 1: إرسال طلب جديد
                    friendActionHTML = `
                        <button class="btn-primary" id="profile-add-friend-btn" style="width: 100%; margin-top: 15px; box-shadow: 0 0 15px rgba(0,242,167,0.2);" onclick="sendFriendRequestFromProfile('${targetUid}')">
                            <i class="fa-solid fa-user-plus"></i> ${t.send_request || (lang === 'en' ? 'Send Friend Request' : 'إرسال طلب صداقة')}
                        </button>
                    `;
                }
            }
        ...
        // --- 🛡️ حالة العصابة (Clan) وزر الدعوة ---
        let clanInfoHTML = '';
        if (data.clanId) {
            ...
            clanInfoHTML = `
                <button class="btn-primary" style="width:100%; margin-top:10px; background: rgba(0, 242, 167, 0.1); border: 1px solid var(--primary-color); color:var(--primary-color);" 
                ...
                <section class="profile-header has-cover" style="position: relative; overflow: hidden; padding-top: ${activeCover ? '0' : '30px'}; margin-bottom: 25px; border-radius: 30px; background: linear-gradient(180deg, rgba(0, 242, 167, 0.1) 0%, transparent 100%); animation: fadeIn 0.4s;">
                    ...
                    <div class="profile-header-content" style="position: relative; z-index: 2; width: 100%; display: flex; flex-direction: column; align-items: center; padding-top: ${activeCover ? '80px' : '0'};">
                        ...
                        <div class="bio-container" style="margin-top: 10px;">
                            ...
                            <p style="color: var(--slate); font-size: 0.8rem; margin-bottom: 5px;">
                                <i class="fa-solid fa-dumbbell"></i> ${data.gym || t.no_gym}
                            </p>
                            
                            <!-- ظهور أزرار الصداقة ثم أزرار الكلان -->
                            ${friendActionHTML}
                            ${clanInfoHTML}
...
window.toggleMemberMenu = function(menuId, btnElement) {
    ...
    // إغلاق كل القوائم
    document.querySelectorAll('.dropdown-content-pro').forEach(m => {
    ...
    if (!event.target.matches('.dots-btn') && !event.target.matches('.fa-ellipsis-vertical')) {
        ...
        for (let i = 0; i < dropdowns.length; i++) {
            ...
            if (openDropdown.classList.contains('show')) {
                ...
                openDropdown.style.display = 'none'; // سطر إخفاء إضافي عشان ما تعلق
            }
        }
    }
};

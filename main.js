...
window.requestNotificationPermission = async function() {
    try {
        ...
        if (permission === 'granted') {
            showToast("✅ تم تفعيل الإشعارات بنجاح! ستصلك التحديات حتى والتطبيق مغلق.");
            
            // ربط المستخدم بالـ Service Worker للحصول على Token
            if ('serviceWorker' in navigator) {
                const registration = await navigator.serviceWorker.ready;
                
                // هذه الخطوة هي التي تعطيك الـ Token لترسل الإشعار من السيرفر (تحتاج VAPID Key حقيقي لاحقاً)
                /*
                const subscription = await registration.pushManager.subscribe({
                    userVisibleOnly: true,
                    applicationServerKey: urlBase64ToUint8Array(PUBLIC_VAPID_KEY)
                });
                console.log("هذا هو الكود الذي ستحفظه في قاعدة البيانات لترسل الإشعارات لهذا المستخدم:", JSON.stringify(subscription));
                */
               
                // إرسال إشعار ترحيبي محلي فوري كإثبات
                registration.showNotification("RGA Fit 🥇", {
                    body: "عاش يا وحش! أنت الآن متصل بنظام الإشعارات الذكي.",

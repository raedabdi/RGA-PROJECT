...
async function approveWorkout(docId) {
    ...
    try {
                    ...
                    if (currentWarId) {
                        ...
                        if (warContributions.count < 2) {
                            ...
                            if (warDoc.exists && warDoc.data().status === 'active') {
                                ...
                                updatePayload.warContributions = warContributions; // بنحفظها بحساب اللاعب مع التحديثات
                            }
                        }
                    }
                }
            } catch(err) { console.error("Error updating team score on admin approve:", err); }
        }
        ...
        // 1. التحديث الآلي في قاعدة البيانات للاعب
...
// ==========================================
// 🛡️ فتح بوابة العصابات (نسخة مدرعة ضد التعليق)
// ==========================================
window.openGuildHub = async function() {
    ...
    // حماية: إذا لم يتم تحميل الحساب بعد
    if (!user) {
    ...
    try {
        // 🧠 معالجة ذكية: نجلب بيانات اللاعب الحقيقية من الفايربيس بدال ما نعتمد عالجهاز بس
        const userDoc = await db.collection('users').doc(user.uid).get();
        if (userDoc.exists) {
        ...
        renderNoGuildScreen(t); // إذا صار خطأ، افتح شاشة التأسيس مباشرة عشان ما يعلق
    }
};
...
<h4 style="color:white; margin:0; font-size:0.95rem;">${fullName} ${roleIcon}</h4>
            ...
            let warContent = clan.warStatus === 'in_war' && typeof buildWarScreenHTML === 'function' ? buildWarScreenHTML(clan, t) : buildWarIdleHTML(clan, t, isLeader);
    ...
    // 🌟 كود فحص واستدعاء نافذة الحرب المعلقة
    setTimeout(() => {
        const pendingResult = localStorage.getItem('pendingWarResult');
        if (pendingResult) {
            const result = JSON.parse(pendingResult);
            if (typeof showWarResultUI === 'function') {
                showWarResultUI(result.isWinner, result.isDraw);
                localStorage.removeItem('pendingWarResult');
            }
        }
    }, 600); // 600 جزء من الثانية لضمان إن شاشة العصابة فتحت بالكامل
...
window.initClanChat = function(clanId, myUid) {
    ...
    // جلب الرسائل التي لم يمر عليها 48 ساعة فقط (حذف تلقائي للمستخدم)
    ...
    chatUnsubscribe = db.collection('clans').doc(clanId).collection('chat')
        ...
        .onSnapshot(snapshot => {
            chatBox.innerHTML = '';
            if (snapshot.empty) {
                chatBox.innerHTML = `<div style="text-align:center; color:var(--slate); margin-top:50px;"><i class="fa-solid fa-comments fa-2x"></i><p>لا يوجد رسائل حديثة.</p></div>`;
                return;
            }
...
window.sendClanMessage = async function(clanId, myUid) {
    ...
    let userData = JSON.parse(localStorage.getItem('currentUser') || '{}');
    let senderName = userData.firstName || 'الكابتن';
    ...
    } catch(e) { console.error(e); showToast("فشل إرسال الرسالة!"); }
};
...
window.saveClanSettings = async function(clanId) {
    ...
    if(!newName) { showToast("لا يمكن ترك الاسم فارغاً!"); return; }
    ...
    try {
        ...
        showToast("تم تحديث إعدادات العصابة بنجاح! 👑");
...
// ==========================================
// ⚔️ بناء واجهة الحرب النشطة (نسخة موحدة ونهائية مترجمة)
// ==========================================

window.buildWarIdleHTML = function(clan, t, isLeader) {
    let lang = localStorage.getItem('lang') || 'ar'; // أمان للغة
    ...
    return `
        <div class="war-arena-container">
            ...
            <div class="war-content" style="text-align: center;">
                ...
                <h3 style="color: white; font-weight: 900; margin-bottom: 15px; font-size: 1.5rem;">${isSearching ? (lang==='en'?'Searching for Prey...':'جاري البحث عن فريسة...') : (lang==='en'?'War Room is Idle':'غرفة الحرب هادئة')}</h3>
                <p style="color: #aaa; font-size: 0.9rem; margin-bottom: 25px; line-height: 1.6;">${lang==='en'?'Ready your members. The leader can initiate a search for an enemy clan. Total lifted weights during the 24h war decide the winner.':'جهز أعضائك. يمكن للزعيم بدء البحث عن كلان عدو. إجمالي الأوزان المرفوعة خلال 24 ساعة يحدد المنتصر.'}</p>
                ...
                ${isLeader ? 
                    `<button class="btn-primary" style="width: 100%; padding: 15px; font-size: 1.1rem; ${isSearching ? 'background: rgba(255,255,255,0.1); color:var(--slate); border-color:var(--slate);' : 'background: rgba(255,77,77,0.15); color: #ff4d4d; border-color: #ff4d4d; box-shadow: 0 0 20px rgba(255,77,77,0.3);'}" onclick="searchForWar('${clan.id}')" ${isSearching ? 'disabled' : ''}>
                        <i class="fa-solid ${isSearching ? 'fa-radar' : 'fa-crosshairs'}"></i> ${isSearching ? (lang==='en'?'Scanning Radar...':'الرادار يعمل...') : (t.search_war || 'البحث عن فريسة')}
                    </button>` 
                    : `<p style="color: #FFD700; font-weight: bold; padding: 10px; border: 1px dashed #FFD700; border-radius: 8px; background: rgba(255,215,0,0.1);">${lang==='en'?'Only the Leader can start a war.':'الزعيم فقط من يقرر إعلان الحرب.'}</p>`
                }
            </div>
        </div>
    `;
...
window.buildWarScreenHTML = function(clan, t) {
    // تشغيل مستمع الحرب اللحظي
    setTimeout(() => listenToActiveWar(clan.currentWarId, clan.id), 100);

    // 🌐 إعداد الترجمات الديناميكية للشاشة
    const lang = localStorage.getItem('lang') || 'ar';
    const kgLabel = lang === 'en' ? 'Lifted KG' : 'KG المرفوع';
    const warActiveTitle = t.war_active || (lang === 'en' ? 'Bloodbath in Progress!' : 'حرب طاحنة جارية!');
    ...
    return `
        <div class="war-arena-container">
            ...
            <div class="war-content">
                ...
                <!-- ⏳ العداد التنازلي المفقود (تم إرجاعه وتنسيقه) -->
                ...
                <div class="epic-vs-board">
                    ...
                    <div class="clan-side-war mine">
                        ...
                        <div class="clan-tag-war" id="war-mine-tag" style="color:var(--slate); font-size:0.85rem; font-weight:bold; margin-bottom:10px; letter-spacing:1px;">[...]</div>
                        
                        <div class="clan-score-box">
                            <div id="war-score-mine" class="clan-score-war">0</div>
                            <span class="score-label">${kgLabel}</span> <!-- 🟢 تم ربط الترجمة هنا -->
                        </div>
                    </div>
                    ...
                    <div class="clan-side-war enemy">
                        ...
                        <div class="clan-tag-war" id="war-enemy-tag" style="color:var(--slate); font-size:0.85rem; font-weight:bold; margin-bottom:10px; letter-spacing:1px;">[...]</div>
...
window.createGuild = async function() {
    ...
    if (!name || tag.length < 2) { 
        showToast(currentLang === 'en' ? "Enter a valid Name and Tag!" : "أدخل اسم صحيح ورمز (Tag)!"); 
        return; 
    }
    ...
    try {
        ...
        // 🚨 السحر هنا: استخدام setTimeout صغير جداً لضمان أن الفايربيس قد سجل البيانات فعلاً
        // ثم استدعاء دالة التحميل المباشرة بدلاً من openGuildHub لمنع أي تضارب
        setTimeout(() => {
            if (typeof loadActiveGuild === 'function') {
                // تصفير الواجهة لوضع السبينر
                const area = document.getElementById('guild-content-area');
        ...
        showToast(currentLang === 'en' ? "Error creating clan!" : "حدث خطأ أثناء التأسيس!");
...
// 🔥 دالة الاستماع لنتائج الحرب (مع ذكاء لجلب الأسماء المفقودة)
window.listenToActiveWar = async function(warId, myClanId) {
    if (!warId) return;

    db.collection('wars').doc(warId).onSnapshot(async doc => {
        if (!doc.exists) return;
        const war = doc.data();
        ...
        // 🧠 معالجة ذكية: لو الحرب قديمة والأسماء مش مخزنة فيها، رح يجيبهم من الفايربيس غصب!
        let myRealName = myData.name;
        let enemyRealName = enemyData.name;

        if (!myRealName || !enemyRealName) {
            try {
                let myClanDoc = await db.collection('clans').doc(myData.id).get();
                let enemyClanDoc = await db.collection('clans').doc(enemyData.id).get();
                myRealName = myRealName || (myClanDoc.exists ? myClanDoc.data().name : 'Unknown Clan');
                enemyRealName = enemyRealName || (enemyClanDoc.exists ? enemyClanDoc.data().name : 'Unknown Clan');
            } catch(e) { console.error("Error fetching clan names", e); }
        }

        // تحديث النصوص بالشاشة (الأسماء، الأعضاء، الأوزان)
        const myNameEl = document.getElementById('war-mine-name');
        if (myNameEl) myNameEl.innerText = myRealName;

        const enemyNameEl = document.getElementById('war-enemy-name');
        if (enemyNameEl) enemyNameEl.innerText = enemyRealName;

        const myMembersEl = document.getElementById('war-mine-members');
        if (myMembersEl) myMembersEl.innerText = myData.membersCount || 1;

        const enemyMembersEl = document.getElementById('war-enemy-members');
        if (enemyMembersEl) enemyMembersEl.innerText = enemyData.membersCount || 1;

        const myScoreEl = document.getElementById('war-score-mine');
        if (myScoreEl) myScoreEl.innerText = myData.score.toLocaleString();

        const enemyScoreEl = document.getElementById('war-score-enemy');
        if (enemyScoreEl) enemyScoreEl.innerText = enemyData.score.toLocaleString();
...
// 5. تحديث دالة الطرد (Kick) لتفريغ clanId من ملف المطرود
window.kickClanMember = async function(clanId, memberUid) {
    if(!confirm("طرد هذا اللاعب من الكلان؟")) return;
    try {
        const clanRef = db.collection('clans').doc(clanId);
        const doc = await clanRef.get();
        ...
        await clanRef.update({ members: members });
        await db.collection('users').doc(memberUid).update({ clanId: null }); // تحديث داتا اللاعب المطرود
...
// 🌟 دالة بدء الحرب وربط الكلانين ببعض
async function startWarBetweenClans(myClanId, myClanData, enemyClanId, enemyClanData) {
    ...
    // إنشاء ملف المعركة مع تخزين أسماء وعدد أعضاء كل كلان
    batch.set(warRef, {
        id: warId,
        clan1: { 
            id: myClanId, tag: myClanData.tag, name: myClanData.name, 
            membersCount: myClanData.members.length, score: 0 
        },
        clan2: { 
            id: enemyClanId, tag: enemyClanData.tag, name: enemyClanData.name, 
            membersCount: enemyClanData.members.length, score: 0 
        },
        startTime: firebase.firestore.FieldValue.serverTimestamp(),
        status: 'active'
    });

    const myClanRef = db.collection('clans').doc(myClanId);
    batch.update(myClanRef, { warStatus: 'in_war', currentWarId: warId });

    const enemyClanRef = db.collection('clans').doc(enemyClanId);
    batch.update(enemyClanRef, { warStatus: 'in_war', currentWarId: warId });

    await batch.commit();
    showToast(currentLang === 'en' ? "Enemy Found! WAR STARTED!" : "تم العثور على عدو! بدأت الحرب الطاحنة!");
}
...
// ==========================================
// ⚔️ إضافة الوزن لمنافسة الفرق (نظام التمرينين فقط!)
// ==========================================
window.addVolumeToClanWar = async function(volume) {
    try {
        ...
        if (!userData.clanId) return; // إذا اللاعب ذئب وحيد، تجاهل
        ...
        // التحقق مما إذا كان الفريق يخوض منافسة حالياً
        if (clanData.warStatus === 'in_war' && clanData.currentWarId) {
            const currentWarId = clanData.currentWarId;
            
            // 🛡️ فحص عدد التمارين المحسوبة في هذه الحرب حصراً
            let warContributions = userData.warContributions || { warId: null, count: 0 };
            
            // تصفير العداد إذا كانت الحرب جديدة
            if (warContributions.warId !== currentWarId) {
                warContributions = { warId: currentWarId, count: 0 };
            }
            
            // 🛑 إذا سجل تمرينين (لايف أو عادي)، امنعه من إضافة المزيد للحرب
            if (warContributions.count >= 2) {
                ...
                return; // نوقف التنفيذ (لا نزيد نقاط العصابة)
            }
            ...
            if (warDoc.exists && warDoc.data().status === 'active') {
                ...
                // زيادة عداد التمارين وحفظه
                warContributions.count += 1;
                await userRef.update({ warContributions: warContributions });
                
                // تحديث الـ local storage حتى يعمل بسرعة
                let localData = JSON.parse(localStorage.getItem('currentUser') || '{}');
...
window.finishClanWar = async function(warId, myClanId, myScore, enemyScore) {
    ...
    try {
        ...
        // 1. منع التكرار القاطع: إذا كانت الحرب منتهية مسبقاً لا تفعل شيء
        ...
        const myClanRef = db.collection('clans').doc(myClanId);
        batch.update(myClanRef, { warStatus: 'idle', currentWarId: null });

        if (warDoc.exists) {
            const warData = warDoc.data();
            const enemyClanId = warData.clan1.id === myClanId ? warData.clan2.id : warData.clan1.id;
            const enemyClanRef = db.collection('clans').doc(enemyClanId);
            batch.update(enemyClanRef, { warStatus: 'idle', currentWarId: null });
        }

        const clanDoc = await myClanRef.get();
        const members = clanDoc.data().members || [];

        if (isWinner && !isDraw) {
            batch.update(myClanRef, { 'stats.wins': firebase.firestore.FieldValue.increment(1) });
        }

        // 🛡️ الحل السحري لمنع تكرار الإشعارات نهائياً (دمج بالـ merge)
        members.forEach(m => {
        ...
        // 2. حفظ النتيجة في الموبايل
        localStorage.setItem('pendingWarResult', JSON.stringify({ isWinner, isDraw }));

        // 3. عرض النافذة فوراً إذا اللاعب داخل العصابة حالياً
        if (document.getElementById('guild-content-area')) {
            if (typeof showWarResultUI === 'function') {
                showWarResultUI(isWinner, isDraw);
                localStorage.removeItem('pendingWarResult');
            }
        }
...
// ==========================================
// 🎇 إظهار شاشة الفوز / التعادل / الخسارة الفخمة
// ==========================================
window.showWarResultUI = function(isWinner, isDraw) {
    // 🛡️ الحارس المعدل: هل اللاعب فاتح شاشة "عصابات الحديد" الآن؟
    const guildScreen = document.getElementById('guild-content-area');
    
    if (!guildScreen) {
        // اللاعب مش فاتح العصابة (بيلعب أو بالرئيسية)، احفظ النتيجة ووقف
        localStorage.setItem('pendingWarResult', JSON.stringify({ isWinner, isDraw }));
        return; 
    }

    // فحص إذا النافذة طالعة أصلاً عشان ما تتكرر فوق بعض
    ...
    overlay.innerHTML = `
        <div class="epic-war-card">
            ...
            <button onclick="closeWarResultAndRestart()" class="epic-war-btn">
                <i class="fa-solid fa-person-military-pointing"></i> ${lang === 'en' ? "Back to Base" : "العودة للمقر"}
            </button>
        </div>
    `;
...
window.closeWarResultAndRestart = function() {

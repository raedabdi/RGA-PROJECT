...
window.deleteFriendFromProfile = async function(friendId) {
    ...
    if (confirm(confirmMsg)) {
        try {
            // 1. تحديث الـ LocalStorage فوراً (قبل الداتابيس لضمان السرعة)
            let localData = JSON.parse(localStorage.getItem('currentUser') || '{}');
            if (localData.myFriendsList) {
                localData.myFriendsList = localData.myFriendsList.filter(f => f.id !== friendId);
                localStorage.setItem('currentUser', JSON.stringify(localData));
            }

            // 2. إعادة رسم البروفايل فوراً (سيظهر زر "إضافة صديق" لأننا حذفناه من اللوكال ستورج)
...
window.sendFriendRequest = async function(targetUid) {
    ...
    try {
        // تحويل شكل الزر يدوياً فوراً قبل حتى ما يوصل السيرفر (لسرعة الاستجابة)
        ...
        // إعادة رسم البروفايل ليتأكد الـ JS أن الطلب موجود ويظهر زر "تم الإرسال" الرمادي
        setTimeout(() => viewPlayerProfile(targetUid), 500);

...
window.deleteAdminMessage=async function(docId){if(!confirm("حذف الرسالة نهائياً؟"))return;try{await db.collection('contact_messages').doc(docId).delete();showToast("تم حذف الرسالة.");loadAdminMessages()}catch(e){showToast("فشل الحذف!")}};let isApprovingWorkout=!1;async function approveWorkout(docId){const t=translations[currentLang||'ar'];if(!confirm(t.confirm_approve))return;if(isApprovingWorkout)return;isApprovingWorkout=!0;try{showToast(t.processing_wait);const docRef=db.collection('pending_workouts').doc(docId);const docSnap=await docRef.get();if(!docSnap.exists){isApprovingWorkout=!1;return}
const data=docSnap.data();const userRef=db.collection('users').doc(data.userId);const userSnap=await userRef.get();let userData=userSnap.data();let workouts=userData.workouts||[];workouts.unshift({date:data.date,type:data.type,details:data.details});let oldMaxW=userData.stats?.maxWeight||0;let maxW=oldMaxW;data.details.forEach(ex=>{if(parseFloat(ex.weight)>maxW)maxW=parseFloat(ex.weight);});let updatedStats=userData.stats||{};updatedStats.maxWeight=maxW;let dethronedVictim=null;let rankLostName="Fallen Hero";if(maxW>oldMaxW&&userData.city){const top3Snap=await db.collection('users').where('city','==',userData.city).orderBy('stats.maxWeight','desc').limit(3).get();let top3=[];top3Snap.forEach(doc=>top3.push({id:doc.id,...doc.data()}));for(let i=0;i<top3.length;i++){let rival=top3[i];let rivalWeight=rival.stats?.maxWeight||0;if(maxW>rivalWeight&&rival.id!==data.userId&&oldMaxW<=rivalWeight){dethronedVictim=rival;if(i===0)rankLostName=currentLang==='en'?"City Monster":"وحش المدينة";else if(i===1)rankLostName=currentLang==='en'?"Silver Guard":"حارس فضي";else if(i===2)rankLostName=currentLang==='en'?"Bronze Guard":"حارس برونزي";break}}}
let totalVol=0;let totalReps=0;data.details.forEach(ex=>{totalVol+=(parseFloat(ex.weight)*parseInt(ex.reps))||0;totalReps+=parseInt(ex.reps)||0});if(userData.clanId){try{const clanDoc=await db.collection('clans').doc(userData.clanId).get();if(clanDoc.exists&&clanDoc.data().warStatus==='in_war'){const currentWarId=clanDoc.data().currentWarId;if(currentWarId){var warContributions=userData.warContributions||{warId:null,count:0};if(warContributions.warId!==currentWarId){warContributions={warId:currentWarId,count:0}}
if(warContributions.count<2){const warRef=db.collection('wars').doc(currentWarId);const warDoc=await warRef.get();if(warDoc.exists&&warDoc.data().status==='active'){let isClan1=warDoc.data().clan1.id===userData.clanId;let updateField=isClan1?'clan1.score':'clan2.score';let memberField=isClan1?`clan1.members.${data.userId}`:`clan2.members.${data.userId}`;await warRef.update({[updateField]:firebase.firestore.FieldValue.increment(totalVol),[`${memberField}.name`]:userData.firstName||'Hero',[`${memberField}.photo`]:userData.photoURL||'https://i.ibb.co/9mPmHXkh/cropped-circle-image-2.png',[`${memberField}.volume`]:firebase.firestore.FieldValue.increment(totalVol)});warContributions.count+=1}}}}}catch(err){console.error("Clan War Error: ",err)}}
let xpReward=dethronedVictim?1000:50;let updatePayload={workouts:workouts,isWorkoutPending:!1,stats:updatedStats,xp:firebase.firestore.FieldValue.increment(xpReward)};if(dethronedVictim){if(auth.currentUser.uid===data.userId){showToast(currentLang==='en'?"👑 Record Broken! +1000 XP granted.":"👑 تم كسر الرقم! تم منح +1000 XP.")}}
if(typeof warContributions!=='undefined'){updatePayload.warContributions=warContributions}
let userQuests=userData.quests||{active:[],progress:{}};let progressChanged=!1;if(userQuests.active){userQuests.active.forEach(quest=>{if(quest.type==='volume'){userQuests.progress[quest.id]=(userQuests.progress[quest.id]||0)+totalVol;progressChanged=!0}
if(quest.type==='reps'){userQuests.progress[quest.id]=(userQuests.progress[quest.id]||0)+totalReps;progressChanged=!0}
if(quest.type==='workout_days'){userQuests.progress[quest.id]=(userQuests.progress[quest.id]||0)+1;progressChanged=!0}})}
if(progressChanged){updatePayload.quests=userQuests}
await userRef.update(updatePayload);if(dethronedVictim){try{await db.collection('fallen_kings').add({city:userData.city,kingName:dethronedVictim.firstName+" "+(dethronedVictim.lastName||''),kingPhoto:dethronedVictim.photoURL||'https://i.ibb.co/9mPmHXkh/cropped-circle-image-2.png',weight:dethronedVictim.stats?.maxWeight||0,slayerName:userData.firstName+" "+(userData.lastName||''),slayerWeight:maxW,rankLost:rankLostName,fallDate:firebase.firestore.FieldValue.serverTimestamp()})}catch(err){console.error("Valhalla Error:",err)}
const uniqueNotifIdSelf=`throne_win_${docId}_self`;const selfMsg=currentLang==='en'?`👑 You crushed a record in ${userData.city}! You took down a map legend with ${maxW}kg!`:`👑 لقد حطمت عرشاً في ${userData.city}! أنت الآن من أساطير الخريطة بوزن ${maxW}kg!`;await userRef.collection('notifications').doc(uniqueNotifIdSelf).set({type:'throne_win',senderName:t.admin_name||'الإدارة 👑',senderPhoto:'https://i.ibb.co/9mPmHXkh/cropped-circle-image-2.png',text:selfMsg,status:'pending',timestamp:firebase.firestore.FieldValue.serverTimestamp()})}else{await userRef.collection('notifications').doc(`approve_${docId}`).set({type:'admin_alert',senderId:'admin',senderName:t.admin_name,senderPhoto:'https://i.ibb.co/9mPmHXkh/cropped-circle-image-2.png',text:t.admin_notif_approve,status:'pending',timestamp:firebase.firestore.FieldValue.serverTimestamp()})}
if(auth.currentUser&&auth.currentUser.uid===data.userId){let localData=JSON.parse(localStorage.getItem('currentUser')||'{}');localData.stats=updatedStats;localData.isWorkoutPending=!1;localData.xp=(localData.xp||0)+xpReward;localStorage.setItem('currentUser',JSON.stringify(localData))}
try{await storage.refFromURL(data.videoUrl).delete()}catch(err){}
await docRef.delete();showToast(t.approve_success);loadPendingWorkouts()}catch(e){console.error(e);showToast(t.approve_fail)}finally{isApprovingWorkout=!1}}
...
ft_privacy:"سياسة الخصوصية",ft_terms:"شروط الخدمة",ft_community:"سياسات مجتمع الأبطال",legal_privacy_title:"سياسة الخصوصية وحماية البيانات",legal_privacy_body:`
        ...
        <p>في <b>RGA Fitness</b>، نأخذ خصوصيتك على محمل الجد. توضح هذه السياسة كيف نقوم بجمع، استخدام، وحماية بياناتك الشخصية.</p>
...
legal_privacy_body:`
           <p>Last Updated: March 19, 2026</p>
            <p>At <b>RGA Fitness</b>, we take your privacy seriously. This policy explains our data practices.</p>
...
function finishGame(isTimeUp){clearInterval(gameInterval);isGameActive=!1;document.getElementById('game-modal').style.display='none';const savedData=JSON.parse(localStorage.getItem('currentUser')||'{}');const previousBestCombo=savedData.bestCombo||0;const user=auth.currentUser;if(user){if(sessionXP>0){updateQuestProgress('dl_score',sessionXP)}
if(sessionBestCombo>0){updateQuestProgress('dl_combo',sessionBestCombo)}
if(sessionXP>0){showToast(`${translations[currentLang].great_job} +${sessionXP} XP`);addXP(sessionXP,'game','RGA_SECURE_998877','deadlift')}
if(globalBestCombo>previousBestCombo){db.collection('users').doc(user.uid).update({bestCombo:globalBestCombo});savedData.bestCombo=globalBestCombo;localStorage.setItem('currentUser',JSON.stringify(savedData));setTimeout(()=>{showToast(`🔥 رقم قياسي جديد للكومبو: ${globalBestCombo}!`)},2000);updateStat('dl_combo',globalBestCombo,!0)}}}
...
window.addXP=async function(amount,actionType='game',securityToken=null,gameType=null){const user=auth.currentUser;if(!user)return;if(actionType==='game'&&securityToken!=='RGA_SECURE_998877'){console.warn("Unauthorized XP attempt blocked.");return}
try{const secureXPCall=firebase.functions().httpsCallable('secureAddXP');const result=await secureXPCall({actionType:actionType,amount:amount,securityToken:securityToken,gameType:gameType});const xpAddedByServer=result.data.xpAdded;if(xpAddedByServer>0){let savedData=JSON.parse(localStorage.getItem('currentUser')||'{}');const oldLevel=savedData.rank||1;savedData.xp=(savedData.xp||0)+xpAddedByServer;savedData.rank=Math.floor(savedData.xp/500)+1;savedData.maxXp=savedData.rank*500;const now=Date.now();let updates={xp:savedData.xp};if(actionType==='game'&&gameType){if(gameType==='deadlift'){savedData.lastDeadliftTime=now;updates.lastDeadliftTime=now}
...
savedData.lastGameXPTime=now;updates.lastGameXPTime=now}else if(actionType==='meal'){savedData.lastMealTime=now;updates.lastMealTime=now}else if(actionType==='workout'){const todayStr=new Date().toDateString();savedData.lastWorkoutXpDate=todayStr;updates.lastWorkoutXpDate=todayStr}else if(actionType==='tribute'){const todayStr=new Date().toDateString();savedData.lastTributeClaim=todayStr;updates.lastTributeClaim=todayStr}
localStorage.setItem('currentUser',JSON.stringify(savedData));if(typeof renderUI==="function")renderUI(savedData);await db.collection('users').doc(user.uid).update(updates);if(typeof updateStat==="function"){updateStat('xpTotal',xpAddedByServer,!1);if(savedData.rank>oldLevel)updateStat('levelReach',savedData.rank,!0);}
if(savedData.rank>oldLevel){const t=translations[currentLang||'ar'];showToast(`${t.level_up} ${savedData.rank}! `)}}}catch(error){console.error("Critical XP Error:",error.message)}};function checkGameCooldown(gameType){const data=JSON.parse(localStorage.getItem('currentUser')||'{}');const now=Date.now();const t=translations[currentLang||'ar'];const lastAnyGame=data.lastGameXPTime||0;if(now-lastAnyGame<10000){showToast(`${t.wait_setup} (${Math.ceil((10000 - (now - lastAnyGame)) / 1000)}s)`);return!1}
let lastPlayed=0;if(gameType==='deadlift')lastPlayed=data.lastDeadliftTime||0;else if(gameType==='squat')lastPlayed=data.lastSquatTime||0;else if(gameType==='reflex')lastPlayed=data.lastReflexTime||0;const cooldownMs=5*1000;if(now-lastPlayed<cooldownMs){const minsLeft=Math.ceil((cooldownMs-(now-lastPlayed))/60000);showToast(`${t.game_cooldown} ${minsLeft} ${currentLang === 'en' ? 'm' : 'دقيقة'} `);return!1}
return!0}
...
const userPhoto=targetUser.photoURL||"https://i.ibb.co/9mPmHXkh/cropped-circle-image-2.png";const earnedCount=(targetUser.earnedBadges||[]).length;resultContainer.innerHTML=`
...
const data=doc.data();const earnedCount=(data.earnedBadges||[]).length;const bio=data.bio||"💪";const userPhoto=data.photoURL||"https://i.ibb.co/9mPmHXkh/cropped-circle-image-2.png";const activeCover=data.currentCover||'';const coverHtml=activeCover?`<div class="profile-cover-image" style="background-image: url('${activeCover}'); opacity: 0.6;"></div>`:'';const activeBorder=data.currentBorder||'';const activeTitle=data.currentTitle||'';let titleHTML='';if(activeTitle&&typeof storeItemsDB!=='undefined'){const foundTitle=storeItemsDB.titles.find(t=>t.val===activeTitle||t.val_en===activeTitle);if(foundTitle){const isEn=currentLang==='en';const displayTitle=isEn?foundTitle.val_en:foundTitle.val;const badgeContent=isEn?`<i class="${foundTitle.icon}"></i> <span>${displayTitle}</span>`:`<span>${displayTitle}</span> <i class="${foundTitle.icon}"></i>`;titleHTML=`
                    <div class="title-badge ${foundTitle.tier}" style="margin-top: 10px; margin-bottom: 5px; transform: scale(1.15);">
                        ${badgeContent}
                    </div>`}}
...
return}}
await inviteRef.set({title:lang==='en'?"Clan Invitation 🛡️":"دعوة لعصابة حديد 🛡️",text:lang==='en'?`Captain ${myData.firstName} invited you!`:`لقد دعاك الكابتن ${myData.firstName} للانضمام!`,type:"clan_invite",clanId:myData.clanId,senderUid:myUid,senderPhoto:myData.photoURL||"https://i.ibb.co/9mPmHXkh/cropped-circle-image-2.png",status:'pending',timestamp:firebase.firestore.FieldValue.serverTimestamp()});showToast(lang==='en'?'Invitation sent!':'تم إرسال الدعوة بنجاح!');if(btn){btn.innerText=lang==='en'?"Request Sent":"تم إرسال الطلب";btn.style.borderColor="gray";btn.style.color="gray"}}catch(err){console.error(err);showToast(lang==='en'?'Error sending invite':'حدث خطأ في الإرسال');if(btn)btn.disabled=!1}};window.openClanFromInvite=async function(notifId,clanId){const dropdown=document.getElementById('notif-dropdown');if(dropdown)dropdown.classList.remove('show');const profileModal=document.getElementById('player-profile-modal');if(profileModal)profileModal.style.display='none';const currentUser=auth.currentUser;if(currentUser&&notifId){try{await db.collection('users').doc(currentUser.uid).collection('notifications').doc(notifId).update({status:'seen'})}catch(e){console.error(e)}}
...
window.valhallaData=[];snapshot.forEach(doc=>{const data=doc.data();window.valhallaData.push({id:doc.id,kingName:data.kingName,kingPhoto:data.kingPhoto||'https://i.ibb.co/9mPmHXkh/cropped-circle-image-2.png',weight:data.weight,slayerName:data.slayerName,slayerWeight:data.slayerWeight,rawDate:data.fallDate?data.fallDate.toMillis():0,date:data.fallDate?data.fallDate.toDate().toLocaleString('en-GB',{weekday:'short',month:'short',day:'numeric',hour:'2-digit',minute:'2-digit'}):'Unknown'})});window.valhallaData.sort((a,b)=>b.rawDate-a.rawDate);renderGraveyardRows(window.valhallaData)}catch(error){console.error("Valhalla Error:",error);document.getElementById('valhalla-list').innerHTML=`<p style="text-align:center; color:#ff4d4d;">حدث خطأ أثناء استدعاء الأرواح!</p>`}}
...
window.renderStoreItems = function() {
    ...
    const ownedCovers = data.purchasedCovers || []; // الأغلفة لها مصفوفة خاصة
    const inventory = data.inventory || []; // مخزون العناصر القابلة للاستهلاك
    const unlockedClanIcons = data.unlockedClanIcons || []; // أيقونات العصابة المشتراة

    let html = '<div class="store-grid">';
    
    let itemsToRender = [];
    if (currentStoreTab === 'covers') itemsToRender = profileCovers;
    else itemsToRender = storeItemsDB[currentStoreTab] || [];

    // إضافة الثيم الافتراضي إذا كنا في تبويب الثيمات
    ...
    itemsToRender.forEach(item => {
        ...
        if (currentStoreTab === 'covers') {
            ...
            isEquipped = data.currentTitle === item.val;
            type = 'title';
...
window.buyStoreItem = async function(type, itemId, price, val, currency) {
    ...
    if (currentBalance < price) {
        ...
        return; // الخروج من الدالة فوراً إذا كان الرصيد لا يكفي
    }
    ...
    try {
        ...
        if (isXP) {
            ...
            updates.spentXp = data.spentXp;
        } else {
            data.ironCoins = (data.ironCoins || 0) - price;
            updates.ironCoins = data.ironCoins;
        }
        ...
        if (type === 'cover') {
            ...
            updates.purchasedCovers = data.purchasedCovers;
            ...
            updates.unlockedClanIcons = data.unlockedClanIcons;
        } else {
            if (!data.purchasedItems) data.purchasedItems =[];
            data.purchasedItems.push(itemId);
            updates.purchasedItems = data.purchasedItems;
        ...
        console.error(e);
        showToast(currentLang === 'en' ? "Purchase failed!" : "فشل الشراء! حدث خطأ في النظام.");
...
try{const friendsDataPromises=myFriends.map(async(friend)=>{const friendDoc=await db.collection('users').doc(friend.id).get();if(friendDoc.exists){const freshData=friendDoc.data();const freshName=(freshData.firstName+" "+(freshData.lastName||"")).trim();const freshImg=freshData.photoURL||"https://i.ibb.co/9mPmHXkh/cropped-circle-image-2.png";const freshLevel=freshData.rank||1;return `
...
showToast(lang==='en'?"Friendship ended!":"تم إلغاء الصداقة من الطرفين!")}}catch(error){console.error(error);showToast("⚠️ Error deleting")}}};window.acceptFriendRequest=async function(notifId,senderId,senderName,senderPhoto){const currentUser=auth.currentUser;if(!currentUser)return;const myUid=currentUser.uid;const lang=currentLang||'ar';try{document.getElementById(notifId)?.remove();const myRef=db.collection('users').doc(myUid);const senderRef=db.collection('users').doc(senderId);const[myDoc,senderDoc]=await Promise.all([myRef.get(),senderRef.get()]);if(myDoc.exists&&senderDoc.exists){const myData=myDoc.data();const senderData=senderDoc.data();const myFullName=(myData.firstName+" "+(myData.lastName||"")).trim();const senderFullName=(senderData.firstName+" "+(senderData.lastName||"")).trim();const batch=db.batch();batch.update(myRef,{myFriendsList:firebase.firestore.FieldValue.arrayUnion({id:senderId,name:senderFullName,photoURL:senderPhoto})});batch.update(senderRef,{myFriendsList:firebase.firestore.FieldValue.arrayUnion({id:myUid,name:myFullName,photoURL:myData.photoURL||"https://i.ibb.co/9mPmHXkh/cropped-circle-image-2.png"})});batch.delete(myRef.collection('notifications').doc(notifId));await batch.commit();let localData=JSON.parse(localStorage.getItem('currentUser')||'{}');if(!localData.myFriendsList)localData.myFriendsList=[];if(!localData.myFriendsList.find(f=>f.id===senderId)){localData.myFriendsList.push({id:senderId,name:senderFullName,photoURL:senderPhoto});localStorage.setItem('currentUser',JSON.stringify(localData))}
...
isInitialLoad=!1;body.innerHTML='';snapshot.forEach(doc=>{const notif=doc.data();const notifId=doc.id;if(notif.type==='friend_request'){body.insertAdjacentHTML('beforeend',`
                      ...
                      </div>`)}else if(notif.type==='throne_win'){let btnText=currentLang==='en'?"Awesome!":"كفو يا وحش! ";body.insertAdjacentHTML('beforeend',`
        ...
        </div>`)}else if(notif.type==='admin_alert'){let savedData=JSON.parse(localStorage.getItem('currentUser')||'{}');if(savedData.isWorkoutPending){savedData.isWorkoutPending=!1;localStorage.setItem('currentUser',JSON.stringify(savedData))}
...
const warRef=db.collection('wars').doc(currentWarId);const warDocLocal=await warRef.get();if(warDocLocal.exists&&warDocLocal.data().status==='active'){let isClan1=warDocLocal.data().clan1.id===userData.clanId;let updateField=isClan1?'clan1.score':'clan2.score';let memberField=isClan1?`clan1.members.${user.uid}`:`clan2.members.${user.uid}`;await warRef.update({[updateField]:firebase.firestore.FieldValue.increment(volume),[`${memberField}.name`]:userData.firstName||'Hero',[`${memberField}.photo`]:userData.photoURL||'https://i.ibb.co/9mPmHXkh/cropped-circle-image-2.png',[`${memberField}.volume`]:firebase.firestore.FieldValue.increment(volume)});warContributions.count+=1;await userRef.update({warContributions:warContributions});let localData=JSON.parse(localStorage.getItem('currentUser')||'{}');localData.warContributions=warContributions;localStorage.setItem('currentUser',JSON.stringify(localData));if(warContributions.count===2){setTimeout(()=>showToast(currentLang==='en'?"2/2 War workouts used!":"استنفذت تمرينين الحرب (2/2)! التمارين القادمة لن تحسب فيها."),1500)}else{setTimeout(()=>showToast(currentLang==='en'?"1/2 War workouts logged!":"تم إضافة وزنك للحرب (1/2)!"),1500)}}}}catch(err){console.error("خطأ في احتساب النقاط للفريق:",err)}};window.finishClanWar=async function(warId,myClanId,myScore,enemyScore){const lang=localStorage.getItem('lang')||'ar';try{const warRef=db.collection('wars').doc(warId);const warDoc=await warRef.get();if(warDoc.exists&&warDoc.data().status==='completed'){return}
let batch=db.batch();batch.update(warRef,{status:'completed'});const warData=warDoc.data();const clan1Id=warData.clan1.id;const clan2Id=warData.clan2.id;const clan1Ref=db.collection('clans').doc(clan1Id);const clan2Ref=db.collection('clans').doc(clan2Id);batch.update(clan1Ref,{warStatus:'idle',currentWarId:null,lastWarId:warId});batch.update(clan2Ref,{warStatus:'idle',currentWarId:null,lastWarId:warId});const clan1Doc=await clan1Ref.get();const clan2Doc=await clan2Ref.get();const clan1Members=clan1Doc.exists?clan1Doc.data().members||[]:[];const clan2Members=clan2Doc.exists?clan2Doc.data().members||[]:[];const clan1Score=warData.clan1.score||0;const clan2Score=warData.clan2.score||0;const clan1Wins=clan1Score>clan2Score;const clan2Wins=clan2Score>clan1Score;const isDraw=clan1Score===clan2Score;if(clan1Wins)batch.update(clan1Ref,{'stats.wins':firebase.firestore.FieldValue.increment(1)});if(clan2Wins)batch.update(clan2Ref,{'stats.wins':firebase.firestore.FieldValue.increment(1)});const sendWarNotifs=(members,won,draw)=>{members.forEach(m=>{const notifRef=db.collection('users').doc(m.uid).collection('notifications').doc('war_result_'+warId);if(won){batch.set(notifRef,{title:lang==='en'?"🏆 Clan War Victory!":"🏆 انتصار في حرب الكلانات!",text:lang==='en'?"Your clan won the war! You received 1000 XP.":"لقد سحق كلانك الخصوم في الحرب! حصلت على 1000 نقطة خبرة (XP).",type:'system_reward',status:'pending',timestamp:firebase.firestore.FieldValue.serverTimestamp()},{merge:!0})}else if(draw){batch.set(notifRef,{title:lang==='en'?"⚖️ Clan War Draw":"⚖️ تعادل في الحرب",text:lang==='en'?"The war ended in a draw! No XP awarded.":"انتهت الحرب بالتعادل! لا يوجد فائز ولا خاسر.",type:'system_reward',status:'pending',timestamp:firebase.firestore.FieldValue.serverTimestamp()},{merge:!0})}else{batch.set(notifRef,{title:lang==='en'?"💀 Clan War Defeat":"💀 هزيمة في الحرب",text:lang==='en'?"Your clan lost the war. Train harder and seek revenge!":"لقد خسر كلانك الحرب. ارفعوا أوزان أثقل وانتقموا!",type:'system_reward',status:'pending',timestamp:firebase.firestore.FieldValue.serverTimestamp()},{merge:!0})}})};sendWarNotifs(clan1Members,clan1Wins,isDraw);sendWarNotifs(clan2Members,clan2Wins,isDraw);await batch.commit();const myActualWin=(myClanId===clan1Id&&clan1Wins)||(myClanId===clan2Id&&clan2Wins);if(myActualWin&&!isDraw){setTimeout(()=>{if(typeof addXP==="function")addXP(1000,'game','RGA_SECURE_998877','war_win');},1000)}
if(document.getElementById('guild-content-area')){let userData=JSON.parse(localStorage.getItem('currentUser')||'{}');userData.lastSeenWarId=warId;localStorage.setItem('currentUser',JSON.stringify(userData));if(auth.currentUser){db.collection('users').doc(auth.currentUser.uid).update({lastSeenWarId:warId})}
showWarResultUI(myActualWin,isDraw,warId,myClanId)}}catch(error){console.error("خطأ في إنهاء الحرب:",error)}};window.closeWarResultAndGoToTab=function(){const resultOverlay=document.getElementById('war-result-overlay');if(resultOverlay)resultOverlay.remove();const guildContentArea=document.getElementById('guild-content-area');if(guildContentArea){const warTabBtn=document.querySelector('.perf-tab-btn[onclick*="war"]');if(warTabBtn){switchGuildTab('war',warTabBtn)}}else{if(typeof openGuildHub==='function'){openGuildHub();setTimeout(()=>{const warTabBtn=document.querySelector('.perf-tab-btn[onclick*="war"]');if(warTabBtn)switchGuildTab('war',warTabBtn);},1000)}}};window.startBackgroundWarMonitor=async function(user){if(!user)return;const userDoc=await db.collection('users').doc(user.uid).get();if(!userDoc.exists)return;const userData=userDoc.data();if(userData.clanId){db.collection('clans').doc(userData.clanId).onSnapshot(clanDoc=>{if(!clanDoc.exists)return;const clanData=clanDoc.data();if(clanData.warStatus==='in_war'&&clanData.currentWarId){db.collection('wars').doc(clanData.currentWarId).onSnapshot(warDoc=>{if(!warDoc.exists)return;const warData=warDoc.data();if(warData.status==='active'&&warData.startTime){const warDurationMs=15*1000;const startTimeMs=typeof warData.startTime.toDate==='function'?warData.startTime.toDate().getTime():new Date(warData.startTime).getTime();const endTime=startTimeMs+warDurationMs;const distance=endTime-new Date().getTime();let myData=warData.clan1.id===userData.clanId?warData.clan1:warData.clan2;let enemyData=warData.clan1.id===userData.clanId?warData.clan2:warData.clan1;if(distance<=0){finishClanWar(warData.id,userData.clanId,myData.score,enemyData.score)}else{if(window.bgWarTimer)clearTimeout(window.bgWarTimer);window.bgWarTimer=setTimeout(()=>{finishClanWar(warData.id,userData.clanId,myData.score,enemyData.score)},distance)}}})}})}};window.buildActiveWarHTML=function(warData,myClanId){let lang=localStorage.getItem('lang')||'ar';let myData=warData.clan1.id===myClanId?warData.clan1:warData.clan2;let enemyData=warData.clan1.id===myClanId?warData.clan2:warData.clan1;const renderMembers=(clanData)=>{let membersArr=[];if(clanData.members){for(let uid in clanData.members){membersArr.push(clanData.members[uid])}}
membersArr.sort((a,b)=>b.volume-a.volume);if(membersArr.length===0)return `<p style="color:var(--slate); font-size:0.8rem; text-align:center;">${lang==='en'?'No data':'لا توجد بيانات'}</p>`;return membersArr.map((m,i)=>`
            <div style="display:flex; justify-content:space-between; align-items:center; background:rgba(255,255,255,0.05); padding:8px 12px; border-radius:8px; margin-bottom:5px;">
                <div style="display:flex; align-items:center; gap:10px;">
                    <span style="color:var(--slate); font-weight:bold; font-size:0.8rem;">#${i+1}</span>
...
const clan=doc.data()||{};const isLeader=clan.leaderId===myUid;let clanName=clan.name||'عصابة مجهولة';let clanTag=clan.tag||'000';let clanMotto=clan.motto||(lang==='en'?'No motto set yet.':'لم يتم تعيين عبارة للكلان.');let clanEmblem=clan.emblem||'fa-solid fa-dragon';let clanColor=clan.emblemColor||'#FFD700';let clanWins=clan.stats?clan.stats.wins:0;let clanMembers=Array.isArray(clan.members)?clan.members:[];let membersPromises=clanMembers.map(async m=>{if(!m||!m.uid)return'';let isMng=m.uid===clan.leaderId;let roleIcon=isMng?`<i class="fa-solid fa-crown" style="color:${clanColor}; filter: drop-shadow(0 0 5px ${clanColor});"></i>`:'';let userPhoto="https://i.ibb.co/9mPmHXkh/cropped-circle-image-2.png";let fullName=m.name||'بطل';try{let uDoc=await db.collection('users').doc(m.uid).get();if(uDoc.exists){let uData=uDoc.data();if(uData.photoURL)userPhoto=uData.photoURL;if(uData.firstName)fullName=uData.firstName+" "+(uData.lastName||"");}}catch(e){}
...
window.handleReflexTap=function(laneIndex,event){if(event)event.preventDefault();if(!isReflexActive)return;const board=document.getElementById('reflex-board');const boardRect=board.getBoundingClientRect();const tapY=(event.touches?event.touches[0].clientY:event.clientY)-boardRect.top;const targetTile=reflexTilesData.find(t=>t.lane===laneIndex&&!t.tapped);if(targetTile&&tapY>=targetTile.y-25&&tapY<=targetTile.y+TILE_HEIGHT+25){targetTile.tapped=!0;playTrueSymphonyNote(reflexScore);if(navigator.vibrate)navigator.vibrate(15);if(targetTile.isGold&&reflexLives<3){reflexLives++;updateLivesUI()}
const laneEl=document.querySelectorAll('.reflex-lane')[laneIndex];const hitX=laneEl.getBoundingClientRect().width/2;const hitEffect=document.createElement('div');hitEffect.className='hit-effect';hitEffect.style.left=hitX+'px';hitEffect.style.top=tapY+'px';const computedStyle=window.getComputedStyle(board);const currentColor=computedStyle.color||'white';hitEffect.style.color=currentColor;laneEl.appendChild(hitEffect);setTimeout(()=>hitEffect.remove(),300);targetTile.element.remove();reflexTilesData=reflexTilesData.filter(t=>t.id!==targetTile.id);reflexScore++;reflexSpeed+=0.012;document.getElementById('reflex-score-val').innerText=reflexScore;document.getElementById('reflex-speed-val').innerText='x'+(reflexSpeed/4.5).toFixed(1)}else{reflexLives--;updateLivesUI();currentNoteIndex=0;board.classList.add('reflex-board-shake');setTimeout(()=>board.classList.remove('reflex-board-shake'),400);if(navigator.vibrate)navigator.vibrate(50);if(reflexLives<=0)endReflexGame(!1);}};function endReflexGame(userQuit){isReflexActive=!1;cancelAnimationFrame(reflexGameLoopId);setTimeout(()=>{document.getElementById('reflex-game-modal').style.display='none';let earnedXP=Math.floor(reflexScore/2);if(earnedXP>150)earnedXP=150;if(earnedXP>0){showToast(`${translations[currentLang].game_over} +${earnedXP} XP`);addXP(earnedXP,'game','RGA_SECURE_998877','reflex');updateStat('reflex_score',reflexScore,!0)}},userQuit?0:800)}

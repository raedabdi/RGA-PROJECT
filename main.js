...
async function approveWorkout(docId) {
    ...
    try {
        ...
        let xpReward = dethronedVictim ? 1000 : 50;
...
if (dethronedVictim) {
            ...
            if (auth.currentUser.uid === data.userId) {
                showToast(currentLang === 'en' ? "👑 Record Broken! +1000 XP granted." : "👑 تم كسر الرقم! تم منح +1000 XP.");
            }
        }
...
// تحديث بيانات التخزين المحلي للآدمن (لو كان يختبر بحسابه)
        if (auth.currentUser && auth.currentUser.uid === data.userId) {
            ...
            localData.xp = (localData.xp || 0) + xpReward; // نستخدم المتغير الجديد هنا
...
// ==========================================
// 🕹️ نظام صالة التحديات (Final Polished Arcade Lobby)
// ==========================================

window.openGameSelection = function() {
    const modal = document.getElementById('game-selection-modal');
    
    applyLanguage(); // لترجمة "العب الآن"
...
window.closeGameSelection = function() {
    const modal = document.getElementById('game-selection-modal');
    modal.classList.remove('active'); 
    
    // 🔓 فك القفل لترجع الشاشة تتحرك
...
// 💡 نافذة المساعدة الهولوغرامية (بتصميم زجاجي شفاف)
window.showUltimateHelp = function(gameType) {
    const t = translations[currentLang || 'ar'];
    let title = "", text = "", color = "", icon = "";

    if (gameType === 'deadlift') {
        title = t.help_deadlift_title;
        text = t.help_deadlift_text;
        color = "#ff4d4d"; icon = "fa-arrow-up-right-dots";
    } else if (gameType === 'squat') {
        title = t.help_squat_title;
        text = t.help_squat_text;
        color = "#00f2a7"; icon = "fa-arrows-down-to-line";
    } else if (gameType === 'reflex') {
        title = t.help_reflex_title;
        text = t.help_reflex_text;
        color = "#ff9f43"; icon = "fa-bolt-lightning";
    }

    const modalHtml = document.createElement('div');
    modalHtml.className = 'modal-overlay active';
    modalHtml.style.zIndex = '65000'; 
    
    // ✨ السر هنا: خلفية شفافة جداً مع ضبابية عالية لتظهر الخلفية
    modalHtml.style.background = 'rgba(5, 10, 20, 0.5)';
    modalHtml.style.backdropFilter = 'blur(15px)';
...
maintenance_desc: "نحن نعمل على ترقية الأنظمة لتقديم تجربة أفضل. نعتذر عن الإزعاج، يرجى المحاولة لاحقاً.",
...
// --- الميزات (Features) ---
feat1_title: "نظام وحش المدينة",
feat1_desc: "انضم إلى ساحة القتال! تنافس مع أصدقائك في مدينتك، حطم الأرقام القياسية، واجلس على عرش الصدارة. عند كسر الرقم، نطلب إثبات فيديو، وعندما تُثبت قوتك، تصبح الملك المطلوب للعدالة! هل أنت مستعد للسيطرة؟",
feat2_title: "رادار العضلات والجاهزية",

rga_way_1_title: "مدرب ذكي (AI) يراقبك",
rga_way_1_desc: "نظام ذكاء اصطناعي يحلل أداءك، يتوقع أقصى وزن لك (1RM)، ويحذرك من الإرهاق العصبي.",
rga_way_2_title: "عصابات، حروب، وجوائز",
rga_way_2_desc: "أسس عصابتك، أعلن الحرب على أندية أخرى، واغنموا نقاط الـ XP والجوائز الملحمية.",
rga_way_3_title: "العب وسيطر على مدينتك",
rga_way_3_desc: "العب تحديات موسيقية، حطم أرقام مدينتك لتصبح 'المطلوب الأول'، واجمع أوسمة نادرة.",

feat1_title: "المدرب الذكي (AI Coach)",
feat1_desc: "خوارزميات ذكاء اصطناعي متطورة تحلل كل تمرين، تقرأ مستوى الإرهاق العصبي (TSB)، وتخبرك باللحظة المثالية لكسر رقمك القياسي مع رادار يراقب توازن عضلاتك.",
feat2_title: "حروب العصابات (Iron Guilds)",
feat2_desc: "الذئاب المنفردة لا تنجو هنا. أسس عصابتك أو انضم لكلان، جهز مقاتليك، وأعلن الحرب الطاحنة ضد العصابات الأخرى لرفع الأوزان وسحق الخصوم لغنم الجوائز.",
...
save_name: "حفظ الاسم الجديد",
name_cooldown: "تغيير الاسم متاح كل 30 يوم فقط!",

comparison_title: "توقف عن التدريب، وابدأ السيطرة",
comparison_desc: "هناك طريقتان للتمرين. الطريقة التقليدية، وطريقة الأبطال.",
old_way_title: "الطريقة القديمة",
old_way_1_title: "تدريب عشوائي",
old_way_1_desc: "تمرين بدون خطة واضحة، أوزان منسية، وتقدم بطيء.",
old_way_2_title: "ملل وفقدان شغف",
old_way_2_desc: "تكرار نفس الروتين يؤدي حتماً إلى فقدان الحماس والتوقف.",
old_way_3_title: "رحلة فردية",
old_way_3_desc: "لا يوجد مجتمع يدعمك، لا منافسة تحفزك، ولا أحد يعترف بك.",
rga_way_title: "طريقة RGA",
rga_way_1_title: "أداء محسوب بالذكاء",
rga_way_1_desc: "رادار العضلات يمنع الإصابات، والنظام يخبرك متى تكسر رقمك.",
rga_way_2_title: "متعة وتحدي مستمر",
rga_way_2_desc: "حوّل تمرينك إلى لعبة، واجمع النقاط لتتصدر الترتيب.",
rga_way_3_title: "مجتمع من الوحوش",
rga_way_3_desc: "سيطر على مدينتك، انضم لعصابات الحديد، وافرض اسمك عالمياً.",
location_cooldown: "تغيير الموقع متاح كل 24 ساعة فقط!",
edit_name: "تعديل الاسم",

reflex_title: "صائد الأوزان",
reflex_desc: "اختبر سرعة بديهتك وتركيزك",
reflex_multiplier: "السرعة: ",
reflex_start: "اضغط للبدء",

play_now: "العب الآن",
help_deadlift_title: "تحدي الديدليفت",
help_deadlift_text: "الهدف هو تجميع أكبر عدد من النقاط خلال 60 ثانية. كل ضغطة تمثل رفعة. استمر بالضغط المتواصل والسريع لبناء 'كومبو' يزيد من حماسك. نصيحة احترافية: استخدم إصبعين بالتناوب لتحقيق أقصى سرعة!",
help_squat_title: "تحدي السكوات",
help_squat_text: "التحدي هنا هو الدقة والتركيز. هدفك هو إيقاف الخط المتحرك داخل 'المنطقة الخضراء' تماماً. كل ضغطة صحيحة هي عدة مثالية. احذر، لديك 5 محاولات خاطئة فقط قبل أن ينتهي التحدي. ركز على إيقاع حركة الخط لتتقنها.",
help_reflex_title: "تحدي صائد الأوزان",
help_reflex_text: "مصيرك يعتمد على سرعة بديهتك! المس 'الحديدة' النازلة قبل أن تصل إلى الأسفل. إذا لمست مساراً فارغاً أو تركت حديدة تسقط، ستخسر روحاً. انتبه للـ'القلب الذهبي' النادر، فهو يمنحك روحاً إضافية (بحد أقصى 3). نصيحة: ركز على أعلى الشاشة لتتوقع الوزن القادم.",
...
muscle_arms: "أذرع",
        ...
        legal_privacy_body: `
            ...
            <p>في <b>RGA Fitness</b>، نأخذ خصوصيتك على محمل الجد. توضح هذه السياسة كيف نقوم بجمع، استخدام، وحماية بياناتك الشخصية أثناء استخدامك لمنصتنا.</p>
            ...
            <p>نستخدم أحدث تقنيات التشفير (Secure Servers) لضمان عدم وصول أي طرف غير مصرح له إلى بياناتك أو رسائلك الخاصة في "مجتمع الأبطال".</p>
            <h3>4. حقوقك</h3>
...
new_max_alert: " دمار شامل! كسرت رقمك بنفس التمرين. الإثبات رح ينطلب للوزن الأعلى ({weight}kg)!",
        ...
        corp_about_title: "عن RGA Fitness",
...
muscle_arms: "Arms",
        ...
        corp_about_title: "About RGA Fitness",
...
comparison_title: "Stop Training, Start Dominating",
comparison_desc: "There are two ways to train. The conventional way, and the hero's way.",
old_way_title: "The Old Way",
old_way_1_title: "Random Training",
old_way_1_desc: "Workouts with no clear plan, forgotten weights, and slow progress.",
old_way_2_title: "Boredom & Burnout",
old_way_2_desc: "Repeating the same routine inevitably leads to loss of motivation.",
old_way_3_title: "A Solitary Journey",
old_way_3_desc: "No community, no competition, and no recognition.",
rga_way_title: "The RGA Way",
rga_way_1_title: "AI-Calculated Performance",
rga_way_1_desc: "Muscle radar prevents injuries, and the system tells you when to break PRs.",
rga_way_2_title: "Endless Fun",
rga_way_2_desc: "Turn your workout into a game and collect XP to top the charts.",
rga_way_3_title: "A Community of Beasts",
rga_way_3_desc: "Dominate your city, join Guilds, and establish your name globally.",
...
soon_btn: "We’ll be right back",
maintenance_title: "Under Maintenance ",
maintenance_desc: "We are upgrading our systems to provide a better experience. We apologize for the inconvenience, please try again later.",
...
play_now: "PLAY NOW",
help_deadlift_title: "Deadlift Challenge",
help_deadlift_text: "The goal is to score as many points as possible within 60 seconds. Every tap is a rep. Tap continuously and rapidly to build your 'combo'. Pro tip: Use two fingers alternately for maximum speed!",
help_squat_title: "Squat Challenge",
help_squat_text: "This challenge is about precision and focus. Your goal is to stop the moving line exactly inside the 'Green Zone'. Every correct tap is a perfect rep. Be careful, you only have 5 mistakes before the challenge ends. Focus on the rhythm of the bar's movement to master it.",
help_reflex_title: "Iron Reflex Challenge",
help_reflex_text: "Your survival depends on your reflexes! Tap the falling 'weight plates' before they reach the bottom. Tapping an empty lane or missing a plate will cost you a life. Watch out for the rare 'Golden Heart' plate; it grants you an extra life (max 3). Tip: Keep your eyes on the top of the screen to anticipate the next plate.",
        terms_title: "Terms of Service",
        terms_desc: "By using the app, you agree not to use any cheats. Any account caught cheating will be banned permanently.",


rga_way_1_title: "Smart AI Coach",
rga_way_1_desc: "AI analyzes your performance, predicts your 1RM, and prevents CNS fatigue.",
rga_way_2_title: "Guilds, Wars & Rewards",
rga_way_2_desc: "Forge a guild, declare war on rival gyms, and loot massive XP and epic rewards.",
rga_way_3_title: "Play & Rule Your City",
rga_way_3_desc: "Play musical minigames, smash city records to become 'Most Wanted', and collect rare badges.",

feat1_title: "Smart AI Coach",
feat1_desc: "Advanced AI algorithms analyze every set, read your CNS fatigue (TSB), and tell you the perfect moment to hit a PR, with a radar tracking muscle balance.",
feat2_title: "Iron Guild Wars",
feat2_desc: "Lone wolves don't survive here. Forge or join an Iron Guild, rally your warriors, and declare all-out war against rival clans to lift heavier, crush enemies, and claim epic loot.",
...
reflex_title: "Iron Reflex",
reflex_desc: "Test your speed and focus",
reflex_multiplier: "SPEED: ",
reflex_start: "TAP TO START",
        calc_reps_ph: "Reps",
...
name_cooldown: "Name change available every 30 days!",
location_cooldown: "Location change available every 24 hours!",
edit_name: "Edit Name",
...
mark_read: "Mark as read & clear",
        ...
        legal_privacy_body: `
            ...
            <p>At <b>RGA Fitness</b>, we take your privacy seriously. This policy explains how we collect, use, and protect your personal data.</p>
            ...
            <p>We use military-grade encryption (Secure Servers) to ensure no unauthorized party can access your data or your encrypted chats in the "Heroes Community".</p>
            <h3>4. Your Rights</h3>
        ...
        legal_terms_body: `
            <p>Welcome to the beast realm. By using RGA Fitness, you fully agree to the following terms:</p>
...
// --- تشغيل عرض الميزات التفاعلي ---
function initInteractiveFeatures() {
    const featureItems = document.querySelectorAll('.feature-item');
    const featureImage = document.getElementById('feature-image');
    if (!featureItems.length || !featureImage) return;

    const observer = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                featureItems.forEach(item => item.classList.remove('is-visible'));
...
// ==========================================
// 🧠 محرك المدرب الذكي (AI Recovery Coach)
// ==========================================

// متغير لحفظ حالة العضلات المنهكة (ليومين سابقين)
let globalTiredMuscles = {}; 

// 💡 دالة مساعدة لترجمة اسم العضلة بناءً على اللغة الحالية
function getTranslatedMuscle(arabicKey, lang) {
    // قاموس الترجمة العكسي (من عربي إلى إنجليزي)
    ...
    if (lang === 'en') {
        return muscleTranslations[arabicKey] || arabicKey;
    } else {
        return arabicKey;
    }
}

window.runAICoach = function() {
    const t = translations[currentLang || 'ar'];
    
    // 1. إصلاح مشكلة قراءة البيانات للاعبين الجدد (حماية من SyntaxError)
    let workouts = [];
    try {
        workouts = JSON.parse(localStorage.getItem('userWorkouts') || '[]');
    } catch(e) {
        console.error("AI Coach Workouts Parse Error", e);
    }

    const coachSection = document.getElementById('ai-coach-section');
    const coachText = document.getElementById('ai-coach-text');
    const coachAction = document.getElementById('ai-coach-action');
    const coachTitle = document.getElementById('ai-coach-title');
    const isEn = currentLang === 'en';

    if (!coachSection) return;

    // 2. إظهار رسالة تحفيزية للاعب الجديد بدلاً من إخفاء الكارت
    if (workouts.length === 0) {
        coachTitle.style.color = "var(--slate)";
        coachText.innerText = isEn ? "AI System Standby. Waiting for your first workout." : "نظام الذكاء الاصطناعي في وضع الاستعداد. بانتظار أول تمرين لك!";
        coachAction.innerHTML = `<i class="fa-solid fa-dumbbell" style="opacity:0.5; margin-right:5px;"></i> ${isEn ? "Log a workout to activate analytics." : "سجل تمرينك الأول لتفعيل التحليلات."}`;
        coachSection.style.display = 'block';
        return;
    }

    // --- 1. تحليل TSB (الجهد العصبي) ---
    let ctl = 0, atl = 0;
    ...
    workouts.forEach(w => {
        ...
        let load = w.details ? w.details.reduce((sum, ex) => sum + (parseFloat(ex.weight) || 0), 0) : 0;
        dailyLoads[key] = (dailyLoads[key] || 0) + load;
    });

    let today = new Date(); today.setHours(0,0,0,0);
    for (let d = new Date(today.getTime() - 60 * 86400000); d <= today; d.setDate(d.getDate() + 1)) {
        let key = `${d.getFullYear()}-${d.getMonth() + 1}-${d.getDate()}`;
        let load = dailyLoads[key] || 0;
        ctl = (load * ctlAlpha) + (ctl * (1 - ctlAlpha));
        atl = (load * atlAlpha) + (atl * (1 - atlAlpha));
    }
    const currentTSB = ctl - atl;

    // --- 2. تحليل العضلات المنهكة (Tired Muscle Logic) ---
    globalTiredMuscles = {};
    // ملاحظة: المفاتيح (Keys) هنا باللغة العربية، وهي ما سيتم استخدامه للبحث في القاموس
    const muscleGroups = { 'صدر': 'chest', 'ظهر': 'back', 'أرجل': 'legs', 'أكتاف': 'shoulders', 'أذرع': 'arms', 'بطن': 'core' };
    
    workouts.forEach(w => {
        if(!w.date) return;
        let wDate = new Date(w.date);
        wDate.setHours(0, 0, 0, 0);
        const diffDays = Math.ceil(Math.abs(today - wDate) / 86400000);
        
        if (diffDays <= 2) { 
            const type = (getTranslatedType(w.type) || w.type || "").toLowerCase(); 
            if (type.includes("صدر") || type.includes("chest") || type.includes("دفع") || type.includes("push") || type.includes("شامل")) globalTiredMuscles['صدر'] = true;
            if (type.includes("ظهر") || type.includes("back") || type.includes("سحب") || type.includes("pull") || type.includes("شامل")) globalTiredMuscles['ظهر'] = true;
            if (type.includes("أكتاف") || type.includes("shoulders") || type.includes("دفع") || type.includes("push") || type.includes("شامل")) globalTiredMuscles['أكتاف'] = true;
            if (type.includes("أذرع") || type.includes("arms") || type.includes("بايسبس") || type.includes("ترايسبس") || type.includes("دفع") || type.includes("سحب") || type.includes("شامل")) globalTiredMuscles['أذرع'] = true;
            if (type.includes("أرجل") || type.includes("legs") || type.includes("شامل")) globalTiredMuscles['أرجل'] = true;
        }
    });

    // --- 3. محرك النصائح (Decision Engine) ---
    let message = "";
    let action = "";
    let titleColor = "#00f2a7";

    // جلب العضلات المتاحة للراحة (المفاتيح بالعربي)
    const freshMusclesNames = Object.keys(muscleGroups).filter(m => !globalTiredMuscles[m]);
    const deadMusclesNames = Object.keys(globalTiredMuscles).filter(m => globalTiredMuscles[m]);

    // تحويل الأسماء للعرض حسب اللغة المختارة
    const freshMuscleDisplay = freshMusclesNames[0] ? getTranslatedMuscle(freshMusclesNames[0], currentLang) : null;
    const deadMusclesDisplay = deadMusclesNames.map(key => getTranslatedMuscle(key, currentLang)).join(', ');


    if (currentTSB < -20) {
        message = isEn ? "Critical CNS Fatigue detected. Your nervous system is redlining." : "تحذير: إرهاق عصبي حاد. جهازك العصبي مستنزف بالكامل حالياً.";
        action = isEn ? "Advice: Complete REST day or very light mobility only. No lifting." : "النصيحة: راحة تامة اليوم أو جلسة إطالة خفيفة جداً. ممنوع رفع الأثقال.";
        titleColor = "#ff4d4d";
    } else if (currentTSB > 10) {
        message = isEn ? "System Prime! Your CNS is fully recovered and ready for war." : "كابتن، طاقتك في الذروة! جهازك العصبي مستعد لتحطيم الأرقام.";
        action = isEn ? `Target: High Intensity on ${freshMuscleDisplay || 'your favorite split'}. Go for a PR!` : `الهدف: تمرين عالي الشدة (يُفضل ${freshMuscleDisplay || 'عضلتك المفضلة'}). وقت كسر الـ PR!`;
        titleColor = "#FFD700";
    } else {
        if (deadMusclesNames.length > 0) {
            message = isEn ? `Muscles (${deadMusclesDisplay}) are still in repair mode.` : `عضلات (${deadMusclesDisplay}) لا تزال في مرحلة الاستشفاء.`;
            action = isEn ? `Recommendation: Avoid them today. Focus on ${freshMuscleDisplay || 'Cardio'}.` : `التوصية: تجنب إرهاقها اليوم. ركز في تمرينك على (${freshMuscleDisplay || 'الكارديو'}).`;
        } else {
            message = isEn ? "Steady state. Body is balanced." : "حالة مستقرة. توازن مثالي بين الإرهاق والاستشفاء.";
            action = isEn ? "Advice: Follow your planned routine with 100% focus." : "النصيحة: التزم بجدولك اليومي بتركيز 100%.";
        }
    }

    // --- 4. تحديث الواجهة ---
    coachText.innerText = message;
    coachAction.innerHTML = `<i class="fa-solid fa-quote-left" style="opacity:0.5; margin-right:5px;"></i> ${action}`;
    coachSection.style.display = 'block';
    coachTitle.style.color = titleColor;
};
...
if (typeof runAICoach === 'function') runAICoach();



            const adminEmail = "raedabdi9@gmail.com"; 
            ...
            // 🛡️ حماية الدوال المفقودة (لكي لا توقف الموقع إذا لم تكن موجودة)
            try { if (typeof listenForNotifications === 'function') listenForNotifications(); } catch(e){}
...
window.addXP = async function(amount, actionType = 'game', securityToken = null, gameType = null) {
    ...
    try {
        ...
        const secureXPCall = firebase.functions().httpsCallable('secureAddXP');
        const result = await secureXPCall({ 
        ...
        if (xpAddedByServer > 0) {
            ...
            if (actionType === 'game' && gameType) {
                ...
                savedData.lastGameXPTime = now;
                updates.lastGameXPTime = now;
            } 
            else if (actionType === 'meal') {
                savedData.lastMealTime = now;
                updates.lastMealTime = now;
            }
            else if (actionType === 'workout') {
                const todayStr = new Date().toDateString();
                savedData.lastWorkoutXpDate = todayStr;
                updates.lastWorkoutXpDate = todayStr;
            }
            else if (actionType === 'tribute') {
                const todayStr = new Date().toDateString();
                savedData.lastTributeClaim = todayStr;
                updates.lastTributeClaim = todayStr;
            }
            ...
            if (typeof updateStat === "function") {
                updateStat('xpTotal', xpAddedByServer, false);
            ...
            if (savedData.rank > oldLevel) {
                const t = translations[currentLang || 'ar'];
                showToast(`${t.level_up} ${savedData.rank}! `);
            }
        }
    } catch (error) {
...
function checkGameCooldown(gameType) {
    ...
    if (now - lastAnyGame < 10000) {
        showToast(`${t.wait_setup} (${Math.ceil((10000 - (now - lastAnyGame)) / 1000)}s)`);
        return false;
    }

    let lastPlayed = 0;
    if (gameType === 'deadlift') lastPlayed = data.lastDeadliftTime || 0;
    else if (gameType === 'squat') lastPlayed = data.lastSquatTime || 0;
    else if (gameType === 'reflex') lastPlayed = data.lastReflexTime || 0; // 👈 قراءة وقت البيانو
...
// --- 1. منطق تغيير الاسم (قيد 30 يوم) ---
    const btnToggleName = document.getElementById('btn-toggle-edit-name');
    const nameEditBox = document.getElementById('edit-name-box');
    const nameDisplay = document.getElementById('display-full-name');

    btnToggleName.onclick = () => {
    ...
    document.getElementById('btn-save-name').onclick = async () => {
        const now = Date.now();
        const lastUpdate = data.lastNameUpdate || 0;
        ...
        await db.collection('users').doc(user.uid).update({ firstName: f, lastName: l, lastNameUpdate: now });
        data.firstName = f; data.lastName = l; data.lastNameUpdate = now;
        localStorage.setItem('currentUser', JSON.stringify(data));
        openProfile(); // ريفريش
        showToast(t.profile_save_success);
    };

    // --- 2. منطق تغيير الموقع (قيد 24 ساعة) ---
    // ابحث عن saveBtn.onclick القديم واستبدله بهذا:
    if(saveBtn) {
        saveBtn.onclick = async () => {
            const now = Date.now();
            const lastLocUpdate = data.lastLocationUpdate || 0;
            const oneDay = 24 * 60 * 60 * 1000;

            const cityChanged = (countrySelect.value !== data.country || citySelect.value !== data.city);

            if (cityChanged && (now - lastLocUpdate < oneDay)) {
                const hrsLeft = Math.ceil((oneDay - (now - lastLocUpdate)) / (1000 * 60 * 60));
                showToast(`${t.location_cooldown} (${hrsLeft}h left)`);
                return;
            }

            const updates = { 
                bio: document.getElementById('bio-input').value.trim(),
                gym: document.getElementById('gym-input').value.trim(),
                country: countrySelect.value,
                city: citySelect.value 
            };
            if(cityChanged) updates.lastLocationUpdate = now;
...
// ==========================================
// 🛠️ نظام الدعم والمساعدة الشامل (مع أنيميشن فخم 🎬)
// ==========================================
// ==========================================
// 🛠️ نظام الدعم الفني والمساعدة (النسخة النهائية المحصنة 🛡️)
// ==========================================

window.openHelpSupport = function() {
    try {
        // 1. منع السكرول في الصفحة الخلفية عند فتح المساعدة
        document.body.style.overflow = 'hidden';

        const safeLang = typeof currentLang !== 'undefined' ? currentLang : (localStorage.getItem('lang') || 'ar');
        const isEn = safeLang === 'en';
        
        let helpModal = document.getElementById('advanced-help-modal');
        
        if (!helpModal) {
            // بناء النافذة مع ستايلات إجبارية (Inline Styles) لكسر أي Rules خارجية
            const html = `
            ...
            helpModal = document.getElementById('advanced-help-modal');
            
            // تعبئة الأسئلة
            populateAdvancedFAQs();
            
            // تعبئة الإيميل تلقائياً
            if (typeof auth !== 'undefined' && auth.currentUser) {
                document.getElementById('adv-contact-email').value = auth.currentUser.email;
            }
        }
        
        // إظهار النافذة بقوة الـ !important البرمجية
...
// 2. دالة الإغلاق (إعادة تفعيل السكرول)
window.closeAdvancedHelp = function() {
    const modal = document.getElementById('advanced-help-modal');
    if (modal) {
...
// 3. دالة التبديل
window.switchAdvHelpTab = function(tab) {
    const faqBtn = document.getElementById('adv-faq-tab');
    const contactBtn = document.getElementById('adv-contact-tab');
    const faqSec = document.getElementById('adv-faq-section');
    const contactSec = document.getElementById('adv-contact-section');

    if (tab === 'faq') {
        faqBtn.style.color = '#00f2a7'; faqBtn.style.background = 'rgba(0,242,167,0.1)'; faqBtn.style.borderColor = '#00f2a7';
        contactBtn.style.color = '#8892b0'; contactBtn.style.background = 'rgba(255,255,255,0.05)'; contactBtn.style.borderColor = 'transparent';
        faqSec.style.display = 'block'; contactSec.style.display = 'none';
    } else {
        contactBtn.style.color = '#00f2a7'; contactBtn.style.background = 'rgba(0,242,167,0.1)'; contactBtn.style.borderColor = '#00f2a7';
        faqBtn.style.color = '#8892b0'; faqBtn.style.background = 'rgba(255,255,255,0.05)'; faqBtn.style.borderColor = 'transparent';
        contactSec.style.display = 'block'; faqSec.style.display = 'none';
    }
};

// 4. دالة الإرسال (إلى contact_messages كما في الرولز)
window.submitAdvancedContact = async function(e) {
    e.preventDefault();
    const btn = document.getElementById('adv-contact-submit-btn');
    const email = document.getElementById('adv-contact-email').value;
    const msg = document.getElementById('adv-contact-message').value;
    ...
    try {
        ...
        alert(isEn ? "Message Sent!" : "تم الإرسال بنجاح!");
        closeAdvancedHelp();
    } catch(err) {
        alert(isEn ? "Error!" : "حدث خطأ!");
    } finally {
        btn.disabled = false;
        btn.innerHTML = `<i class="fa-solid fa-paper-plane"></i> ${isEn ? 'Send Message' : 'إرسال الرسالة'}`;
    }
};

// 5. دالة تعبئة الـ 50+ سؤال
window.populateAdvancedFAQs = function() {
    const container = document.getElementById('adv-faq-container');
    const isEn = (typeof currentLang !== 'undefined' ? currentLang : 'ar') === 'en';
    
    // سحب الأسئلة من appFAQs الموجودة في ملفك الأصلي
    let faqsList = (typeof appFAQs !== 'undefined') ? appFAQs : [];

    container.innerHTML = faqsList.map(faq => `
...
window.confirmDeleteAccount = async function() {
    const isEn = (typeof currentLang !== 'undefined' ? currentLang : (localStorage.getItem('lang') || 'ar')) === 'en';
    const user = auth.currentUser;

    if (!user) {
        alert(isEn ? "No active user found!" : "لم يتم العثور على مستخدم نشط!");
        return;
    }

    // 1. تحديد الكلمة المطلوبة والرسائل حسب اللغة
    const confirmWord = isEn ? "CONFIRM" : "تأكيد";
    const titleMsg = isEn 
        ? `🚨 WARNING: This will permanently delete your entire progress and data.\n\nTo proceed, please type "${confirmWord}" in the box below:` 
        : `🚨 تحذير: سيتم مسح جميع بياناتك وتقدمك نهائياً من النظام.\n\nللمتابعة، يرجى كتابة كلمة "${confirmWord}" في المربع أدناه:`;

    // 2. إظهار نافذة الإدخال (Prompt)
    const userInput = prompt(titleMsg);

    // 3. التحقق من الإدخال
    if (userInput === null) return; // إذا ضغط "إلغاء"

    if (userInput.trim() !== confirmWord) {
        alert(isEn ? "Verification failed! Word doesn't match." : "فشل التحقق! الكلمة غير مطابقة.");
        return;
    }

    // 4. إذا كتب الكلمة صح، نبدأ عملية الحذف والتنظيف
    try {
        const uid = user.uid;
        showToast(isEn ? "Processing permanent deletion..." : "جاري تنفيذ الحذف النهائي...");

        // --- أ: تنظيف البيانات (نفس الكود السابق) ---
        const userDoc = await db.collection('users').doc(uid).get();
        if (userDoc.exists) {
            const userData = userDoc.data();

            // إزالة الصديق من عند الآخرين
            if (userData.myFriendsList) {
                for (const friend of userData.myFriendsList) {
                    try {
                        const friendRef = db.collection('users').doc(friend.id);
                        const fDoc = await friendRef.get();
                        if (fDoc.exists) {
                            const newList = fDoc.data().myFriendsList.filter(f => f.id !== uid);
                            await friendRef.update({ myFriendsList: newList });
                        }
                    } catch (e) {}
                }
            }
            ...
            if (userData.clanId) {
                try {
                    ...
                    if (clanDoc.exists) {
                        let members = clanDoc.data().members || [];
                        let updatedMembers = members.filter(m => m.uid !== uid);
                        if (clanDoc.data().leaderId === uid) {
                            if (updatedMembers.length > 0) {
                                await clanRef.update({ members: updatedMembers, leaderId: updatedMembers[0].uid });
                            } else {
                                await clanRef.delete();
                            }
                        } else {
                            await clanRef.update({ members: updatedMembers });
                        }
                    }
                } catch (e) {}
            }
        ...
        // ب: حذف الفيديوهات المعلقة
        const pendingSnap = await db.collection('pending_workouts').where('userId', '==', uid).get();
        for (const doc of pendingSnap.docs) {
            if (doc.data().videoUrl) {
                try { await storage.refFromURL(doc.data().videoUrl).delete(); } catch (e) {}
            }
            await doc.ref.delete();
        }

        // ج: حذف الوثيقة والحساب
        await db.collection('users').doc(uid).delete();
        await user.delete();

        localStorage.clear();
        alert(isEn ? "Your account has been completely wiped. Farewell!" : "تم مسح حسابك بالكامل. وداعاً أيها البطل!");
        window.location.href = 'index.html';

    } catch (error) {
        console.error("Delete Error:", error);
        if (error.code === 'auth/requires-recent-login') {
            alert(isEn ? "Security Timeout: Please logout, login again, and retry deletion." : "فحص أمني: يرجى تسجيل الخروج والدخول مجدداً ثم المحاولة مرة أخرى.");
        } else {
            alert(isEn ? "An error occurred during deletion." : "حدث خطأ أثناء محاولة الحذف.");
        }
    }
};
...
async function searchPlayerByID() {
    ...
    try {
        ...
        // ✅ التعديل السحري هنا: تمرير الـ ID فقط في زر الإرسال لتجنب خطأ الأسماء التي تحتوي على فواصل
...
async function viewPlayerProfile(targetUid) {
    ...
    // 🔥 أهم سطرين: قراءة بياناتي المحدثة من الذاكرة فوراً لضمان حالة الصداقة بدون ريفرش
    const myUserData = JSON.parse(localStorage.getItem('currentUser') || '{}');
...
modal.style.cssText = "position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; background-color: transparent !important; z-index: 999998; display: flex; flex-direction: column; overflow-y: auto; -webkit-tap-highlight-color: transparent; outline: none;";
...
`;
    ...
    try {
        ...
        const activeCover = data.currentCover || '';
        const coverHtml = activeCover ? `<div class="profile-cover-image" style="background-image: url('${activeCover}'); opacity: 0.6;"></div>` : '';
        const activeBorder = data.currentBorder || '';
        const activeTitle = data.currentTitle || '';
        ...
        let earnedBadgesHTML = `<p style="text-align: center; color: var(--slate); font-size: 0.8rem; padding: 20px;">${t.no_badges_yet || 'لا توجد أوسمة'}</p>`;
        ...
        if (myUid && myUid !== targetUid) {
            ...
            if (isFriend) {
                const friendBtnText = lang === 'en' ? '<i class="fa-solid fa-user-check"></i> Friend ' : '<i class="fa-solid fa-user-check"></i> صديق';
        ...
        // 🔥 تم استبدال الخلفيات الكحلية الصلبة بخلفيات شفافة (transparent) مع حدود فخمة
        modal.innerHTML = `
            <header class="top-bar" style="position: sticky; top: 0; background: transparent; z-index: 10; border-bottom: 1px solid rgba(0, 242, 167, 0.2);">
                <div class="header-row">
                    <button onclick="document.getElementById('player-profile-modal').style.display='none'" class="btn-primary" style="padding: 5px 15px; -webkit-tap-highlight-color: transparent;">${t.back}</button>
...
window.renderClanPreview = async function(clanData, clanId, t) {
    const mainContent = document.getElementById('main-content-area');
    // إذا لم نكن داخل شاشة العصابات، سنستخدم الشاشة الرئيسية
    const area = document.getElementById('guild-content-area') || mainContent; 
    const lang = currentLang || 'ar';
    ...
    area.innerHTML = `<div style="text-align:center; padding: 50px;"><i class="fa-solid fa-spinner fa-spin fa-3x" style="color:var(--primary-color);"></i></div>`;

    try {
        // 2. جلب البيانات إذا كانت مفقودة (لتجنب أي أخطاء)
        if (!clanData) {
            const doc = await db.collection('clans').doc(clanId).get();
            if (!doc.exists) {
                area.innerHTML = `<p style="text-align:center; color:red;">${lang === 'en' ? 'Clan not found!' : 'هذه العصابة لم تعد موجودة!'}</p>`;
                return;
            }
            clanData = doc.data();
        }

        // 3. تجهيز المتغيرات والإحصائيات
        const memberCount = clanData.members ? clanData.members.length : 1;
        const emblem = clanData.emblem || 'fa-solid fa-shield-cat';
        const score = clanData.score || 0;
        const isFull = memberCount >= 50;
        const isMyClan = currentUser.clanId === clanId;

        // 4. تحديد حالة زر (الانضمام / المغادرة)
        let actionBtnHtml = '';
        if (isMyClan) {
            // إذا كانت عصابتي، أعرض زر مغادرة
            actionBtnHtml = `<button class="btn-primary" style="width:100%; padding: 12px; font-size: 1.1rem; background: rgba(255, 77, 77, 0.1); color: #ff4d4d; border-color: #ff4d4d;" onclick="leaveGuild('${clanId}')"><i class="fa-solid fa-person-walking-arrow-right"></i> ${lang === 'en' ? 'Leave Clan' : 'مغادرة العصابة'}</button>`;
        } else if (currentUser.clanId) {
            // إذا كنت في عصابة أخرى
            actionBtnHtml = `<button class="btn-primary" style="width:100%; padding: 12px; font-size: 1.1rem; background: rgba(255, 255, 255, 0.1); color: gray; border-color: gray; cursor: not-allowed;" disabled><i class="fa-solid fa-shield-halved"></i> ${lang === 'en' ? 'You are already in a clan' : 'أنت في عصابة بالفعل'}</button>`;
        } else if (isFull) {
            // إذا كانت العصابة ممتلئة
            actionBtnHtml = `<button class="btn-primary" style="width:100%; padding: 12px; font-size: 1.1rem; background: rgba(255, 255, 255, 0.1); color: gray; border-color: gray; cursor: not-allowed;" disabled><i class="fa-solid fa-ban"></i> ${lang === 'en' ? 'Clan is Full' : 'العصابة ممتلئة 50/50'}</button>`;
        } else {
            // يمكنه الانضمام
            actionBtnHtml = `<button class="btn-primary" style="width:100%; padding: 12px; font-size: 1.1rem; box-shadow: 0 0 15px rgba(0,242,167,0.3);" onclick="confirmJoinGuild('${clanId}')"><i class="fa-solid fa-handshake-angle"></i> ${lang === 'en' ? 'Join Clan' : 'الانضمام للعصابة'}</button>`;
        }

        // 5. بناء قائمة الأعضاء بشكل أنيق
        let membersHtml = '';
        if (clanData.members && clanData.members.length > 0) {
            // ترتيب الأعضاء: الزعيم أولاً، ثم حسب النقاط (XP)
            const sortedMembers = [...clanData.members].sort((a, b) => {
            ...
            sortedMembers.forEach(m => {
                ...
                const roleIcon = isLeader ? '<i class="fa-solid fa-crown" style="color: #FFD700; text-shadow: 0 0 5px rgba(255,215,0,0.5);"></i>' : '<i class="fa-solid fa-user-ninja" style="color: var(--slate);"></i>';
                const roleText = isLeader ? (lang === 'en' ? 'Leader' : 'الزعيم') : (lang === 'en' ? 'Member' : 'عضو');
        ...
        console.error("Error rendering clan preview:", err);
        area.innerHTML = `<p style="text-align:center; color:red;">${lang === 'en' ? 'Error loading clan details.' : 'حدث خطأ أثناء تحميل تفاصيل العصابة.'}</p>`;
    }
};
...
if (freshUserData.lastKickedAt && freshUserData.kickedFromClanId === clanId) {
    const lastKicked = freshUserData.lastKickedAt.toDate();
    const hoursPassed = (new Date() - lastKicked) / (1000 * 60 * 60);
    if (hoursPassed < 24) {
        const waitTime = Math.ceil(24 - hoursPassed);
        showToast(currentLang === 'en' ? `Banned from this clan! Wait ${waitTime} hours.` : `ممنوع الدخول! انتظر ${waitTime} ساعة لتعود لهذه العصابة.`);
        btn.disabled = false;
        btn.innerHTML = originalText;
        return;
    }
...
// منع دخول اللاعب إذا كان موجود أصلاً في الكلان
if (clanData.members.some(m => m.uid === user.uid)) {
    showToast(currentLang === 'en' ? "You are already in this clan!" : "أنت بالفعل عضو في هذه العصابة!");
    return;
}
...
window.sendClanInvite = async function(targetUid) {
    const myUid = auth.currentUser.uid;
    const lang = currentLang || 'ar';
    
    // منع الكبس المتكرر على الزر
    const btn = event.target.closest('button');
    if (btn) {
        btn.disabled = true;
        btn.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i>';
    ...
    try {
        ...
        if (!myData.clanId) return;

        // معرف فريد للدعوة بين هؤلاء الشخصين لمنع التكرار
        const inviteId = `invite_${myUid}_${targetUid}`;
        const inviteRef = db.collection('users').doc(targetUid).collection('notifications').doc(inviteId);
        
        const existingInvite = await inviteRef.get();
        
        // فحص إذا كان قد أرسل دعوة خلال آخر 24 ساعة
        if (existingInvite.exists) {
            const lastSent = existingInvite.data().timestamp?.toDate();
            const now = new Date();
            if (lastSent && (now - lastSent) < (24 * 60 * 60 * 1000)) {
                showToast(lang === 'en' ? "Invite already sent today!" : "لقد أرسلت دعوة بالفعل لهذا البطل اليوم!");
                if (btn) {
                    btn.innerText = lang === 'en' ? "Sent Today" : "تم الإرسال اليوم";
                    btn.style.opacity = "0.5";
                }
                return;
            }
        ...
        await inviteRef.set({
            title: lang === 'en' ? "Clan Invitation 🛡️" : "دعوة لعصابة حديد 🛡️",
            text: lang === 'en' ? `Captain ${myData.firstName} invited you!` : `لقد دعاك الكابتن ${myData.firstName} للانضمام!`,
            type: "clan_invite",
            clanId: myData.clanId,
            senderUid: myUid,
            senderPhoto: myData.photoURL || "https://i.ibb.co/9mPmHXkh/cropped-circle-image-2.png",
            status: 'pending',
            timestamp: firebase.firestore.FieldValue.serverTimestamp()
        });

        showToast(lang === 'en' ? 'Invitation sent!' : 'تم إرسال الدعوة بنجاح!');
        if (btn) {
            btn.innerText = lang === 'en' ? "Request Sent" : "تم إرسال الطلب";
        ...
        console.error(err);
        showToast(lang === 'en' ? 'Error sending invite' : 'حدث خطأ في الإرسال');
        if (btn) btn.disabled = false;
    }
};
window.openClanFromInvite = async function(notifId, clanId) {
    // 1. إخفاء القائمة المنسدلة للإشعارات
    const dropdown = document.getElementById('notif-dropdown');
    if (dropdown) dropdown.classList.remove('show');

    // 2. إخفاء مودال البروفايل فوراً لأنه هو العائق الأساسي
    const profileModal = document.getElementById('player-profile-modal');
    if (profileModal) profileModal.style.display = 'none';

    // 3. تحديث الإشعار في السيرفر ليصبح seen
    ...
    // 4. الانتقال لواجهة الاستطلاع
    closeProfileAndOpenClan(clanId);
};
// ==========================================
...
window.closeProfileAndOpenClan = async function(clanId) {
    // أهم خطوة: إخفاء المودال وتحرير السكرول لضمان إمكانية الكبس
    const profileModal = document.getElementById('player-profile-modal');
    if (profileModal) {
    ...
    // إذا لم يكن هناك محتوى أصلي محفوظ، نحفظ الداشبورد الحالي قبل تغييره
    if (!mainContent.dataset.originalContent) {
        // نأخذ نسخة من المحتوى الحالي لضمان العودة للرئيسية
        const currentHTML = mainContent.innerHTML;
        if (!currentHTML.includes('id="guild-content-area"')) {
            mainContent.dataset.originalContent = currentHTML;
        }
    }

    // بناء الواجهة مع زر رجوع قوي
    mainContent.innerHTML = `
    ...
    // ربط زر الرجوع برمجياً لضمان العمل
    const backBtn = document.getElementById('force-back-btn');
    if (backBtn) {
    ...
    try {
        ...
        if (clanDoc.exists) {
            renderClanPreview(clanDoc.data(), clanId, t);
        }
    } catch (e) { console.error(e); }
};
...
window.claimTribute = async function() {
    ...
    const success = await addXP(100, 'tribute'); 
    
    if (success) {
        showToast(currentLang === 'en' ? "💰 +100 XP King's tribute claimed!" : "💰 تم تحصيل +100 XP ضريبة الملك!");
        updateQuestProgress('tribute', 1);
        localStorage.setItem('lastTributeClaim', new Date().toDateString());
    } else {
        showToast(currentLang === 'en' ? "Claimed today 👑" : "الضريبة محصلة اليوم 👑");
    }
...
window.cityMonsterMaxWeight = 0; // متغير عالمي لحفظ رقم وحش المدينة
...
function startLiveWorkout() {
    ...
    // سحب رقم وحش المدينة بالخلفية لـ AI Prediction
    const userData = JSON.parse(localStorage.getItem('currentUser') || '{}');
    if(userData.city) {
        db.collection('users').where('city', '==', userData.city).orderBy('stats.maxWeight', 'desc').limit(1).get().then(snap => {
            if(!snap.empty) window.cityMonsterMaxWeight = snap.docs[0].data().stats?.maxWeight || 0;
        });
    }
    ...
    // إخفاء رسالة الـ AI إذا كانت موجودة من تمرين سابق
    const oldAiMsg = document.getElementById('ai-live-prediction');
...
window.logLiveSet = function(isSuperset = false) {
    const muscleSelect = document.getElementById('live-muscle-select');
    ...
    // 🕒 1. نظام الراحة الذكي (Smart Rest Timer)
    let restTime = 90; // الافتراضي
    const exLower = exName.toLowerCase();
    
    if (muscleVal === 'أرجل' || muscleVal === 'ظهر' || exLower.includes('squat') || exLower.includes('deadlift')) {
        restTime = weight >= 100 ? 180 : 120; 
    } else if (muscleVal === 'صدر' || exLower.includes('bench')) {
        restTime = weight >= 80 ? 120 : 90; 
    } else {
        restTime = weight >= 40 ? 90 : 60; 
    }

    // إزالة رسالة الـ AI القديمة عشان ما تتكرر
    const oldAiMsg = document.getElementById('ai-live-prediction');
    if(oldAiMsg) oldAiMsg.remove();

    // طباعة الجولة التي لعبها اللاعب
    ...
    // 🤖 2. نظام توقع الذكاء الاصطناعي (AI 1RM Prediction)
    // حساب أقصى وزن لعدة واحدة (1RM) بناءً على معادلة Epley
    let predicted1RM = weight * (1 + (0.0333 * reps));
    predicted1RM = Math.round(predicted1RM);

    // تفعيل الذكاء الاصطناعي إذا كانت العدات 2 أو أكثر، والوزن 20 كجم أو أكثر
    if (reps >= 2 && weight >= 20) {
        let aiMessage = "";
        let isCloseToMonster = false;
        let monsterWeight = window.cityMonsterMaxWeight || 0;

        if (monsterWeight > 0) {
            if (predicted1RM > monsterWeight) {
                isCloseToMonster = true;
                aiMessage = currentLang === 'en' 
                    ? `⚡ AI Prediction: Your 1RM is ~${predicted1RM}kg! Try lifting ${monsterWeight + 2.5}kg next set to dethrone the City Monster!`
                    : `⚡ توقع الـ AI: قوتك الحقيقية ${predicted1RM} كجم! جرب رفع ${monsterWeight + 2.5} كجم الجولة القادمة لاحتلال عرش المدينة 👑!`;
            } else if (predicted1RM >= monsterWeight - 20) {
                aiMessage = currentLang === 'en' 
                    ? `⚡ AI Warning: Your 1RM is ~${predicted1RM}kg. You are very close to the City Record (${monsterWeight}kg)!`
                    : `⚡ تحليل الـ AI: قوتك القصوى تقريباً ${predicted1RM} كجم. أنت قريب جداً من رقم وحش المدينة (${monsterWeight} كجم)!`;
            } else {
                aiMessage = currentLang === 'en'
                    ? `⚡ AI Analysis: Your 1 Rep Max is roughly ${predicted1RM}kg. Keep pushing!`
                    : `⚡ تحليل الـ AI: حدك الأقصى لعدة واحدة هو تقريباً ${predicted1RM} كجم. استمر بالجلد!`;
            }
        } else {
            // إذا لم يكن هناك وحش في المدينة أو فشل جلب الرقم
            aiMessage = currentLang === 'en'
                ? `⚡ AI Prediction: Your 1 Rep Max is approximately ${predicted1RM}kg. Push your limits!`
                : `⚡ توقع الـ AI: حدك الأقصى لعدة واحدة هو تقريباً ${predicted1RM} كجم. حطم أرقامك!`;
        }

        // إدراج رسالة الـ AI مباشرة فوق الجولة
        if (aiMessage) {
            const aiBoxHtml = `
    ...
    if (muscleVal === "أكتاف" || muscleVal === "أذرع") threshold = 50;
    if (muscleVal === "بطن") threshold = 80;
    if (muscleVal === "أرجل") threshold = 120;
    if (muscleVal === "شامل") threshold = 60;

    if (exLower.includes("ديدليفت") || exLower.includes("deadlift")) threshold = 120;
    if (exLower.includes("سكوات") || exLower.includes("squat")) threshold = 140;
    ...
    if (!isSuperset) {
        ...
        startRestTimer(restTime); // هنا يعمل نظام الراحة الذكي
    } else {
...
window.preloadHeavyCovers = function() {

    const allHeavyImages =[
        'Photos/dms.png', 'Photos/g2.png', 'Photos/zoro.png', 'Photos/solo.png',
        'Photos/gymsm.png', 'Photos/ls.png', 'Photos/lufs.png', 'Photos/sd.png',
        'Photos/shoot.png', 'Photos/fg.png', 'Photos/time.png', 'Photos/fghtm.png',
        'Photos/k1.gif', // صورة الملك المتحركة
        'Photos/dead1.png', // خلفية المقبرة
        'https://i.ibb.co/LDQjfGBk/IMG-4236.png', // خلفية الحرب والألعاب
        'https://i.ibb.co/KczkyPZg/IMG-4723.png', // خلفية نتيجة الحرب
        'body.png' // صورة العضلات
    ];


    allHeavyImages.forEach(src => {
        const img = new Image();
        img.src = src;
    });
    
    console.log(" تم تجهيز جميع الصور الفخمة في الخلفية!");
};
...
window.renderNoGuildScreen = async function(t) {
    const area = document.getElementById('guild-content-area');
    if (!area) return;
    const userData = JSON.parse(localStorage.getItem('currentUser') || '{}');
    const lang = currentLang || 'ar'; 
    
    const txtSearchTitle = lang === 'en' ? 'Explore Clans' : 'استكشاف العصابات';
    const txtCreateBtn = lang === 'en' ? 'Create Clan' : 'تأسيس عصابة';
    const txtSearchPlaceholder = lang === 'en' ? 'Enter Clan Tag...' : 'أدخل رمز العصابة (Tag)...';
    const txtTopSuggested = lang === 'en' ? 'Top Suggested Clans' : 'أبرز العصابات المقترحة';
    const txtSearching = lang === 'en' ? 'Searching for warriors...' : 'جاري البحث عن مقاتلين...';
    const txtNoClans = lang === 'en' ? 'No suggested clans found.' : 'لا توجد عصابات مقترحة حالياً.';

    // 👈 تعديل ذكي: إذا اللاعب في كلان، نخفي زر التأسيس ونحط زر يرجعه لعصابته
    let topActionBtnHtml = '';
    if (userData.clanId) {
        topActionBtnHtml = `
    ...
    try {
        let query = db.collection('clans');
        if (userData.country) {
            query = query.where('country', '==', userData.country).limit(5);
        } else {
            query = query.limit(5);
        }
        
        const snap = await query.get();
        const container = document.getElementById('suggested-clans-container');
        container.innerHTML = '';

        if (snap.empty) {
            container.innerHTML = `<p style="text-align:center; color:gray;">${txtNoClans}</p>`;
            return;
        }

        snap.forEach(doc => {
            const clan = doc.data();
            const memberCount = clan.members ? clan.members.length : 1;
            const score = clan.score || 0;
            const emblem = clan.emblem || 'fa-solid fa-shield-cat';
        ...
        const txtError = lang === 'en' ? 'Error loading suggestions.' : 'حدث خطأ في تحميل الاقتراحات.';
        if (document.getElementById('suggested-clans-container')) {
            document.getElementById('suggested-clans-container').innerHTML = `<p style="color:red; text-align:center;">${txtError}</p>`;
        }
    }
};


window.showCreateClanModal = function(t) {
    const area = document.getElementById('guild-content-area');
    const lang = currentLang || 'ar';
    
    const txtBack = t.back || (lang === 'en' ? 'Back' : 'رجوع');
    const txtCreateTitle = t.create_guild || (lang === 'en' ? 'Create Clan' : 'تأسيس عصابة');
    const txtNameLabel = t.guild_name || (lang === 'en' ? 'Clan Name' : 'اسم العصابة');
    const txtTagLabel = t.guild_tag || (lang === 'en' ? 'Clan Tag (3-4 Letters)' : 'رمز العصابة (3-4 أحرف)');
    const txtCreateBtn = t.create || (lang === 'en' ? 'Create Now' : 'تأسيس الآن');
...
window.joinGuildByTagDirect = function(tag) {
    document.getElementById('search-guild-tag').value = tag;
    joinGuildByTag();
};
window.openGuildHub = async function() {
    // 1. تصفير السكرول فوراً (بأول سطر عشان ما يعلق تحت)
    window.scrollTo(0, 0);

    // 2. إغلاق القائمة الجانبية في الموبايل
    if(window.innerWidth < 768) {
        const sidebar = document.getElementById('sidebar');
        if(sidebar) sidebar.classList.add('collapsed');
    }
    ...
    // تصفير سكرول الحاوية نفسها لضمان إنك تطلع لفوق 100%
    mainContent.scrollTop = 0;

    // حفظ المحتوى الأصلي للداشبورد إذا مش محفوظ
    if (!mainContent.dataset.originalContent) {
        mainContent.dataset.originalContent = mainContent.innerHTML;
    }
    ...
    // 🌟 واجهة مقر العصابة خالية من الأخطاء
    mainContent.innerHTML = `
        <header class="top-bar" style="margin-bottom: 20px;">
            <div class="header-row" style="display:flex; justify-content:space-between; width:100%; align-items: center;">
    ...
    try {
        ...
        if (freshUserDoc.exists) {
            const freshUserData = freshUserDoc.data();
            userData.clanId = freshUserData.clanId;
            localStorage.setItem('currentUser', JSON.stringify(userData));
        }

        // توجيه اللاعب: إذا عنده عصابة يفتحها، وإذا لا يفتح شاشة البحث
        let backBtn = document.getElementById('back-to-dash-btn');
        if (userData.clanId) {
            if (backBtn) {
                backBtn.onclick = function() {
                    renderNoGuildScreen(t);
                    // تغيير الكبسة عشان ترجع للداشبورد في المرة اللي بعدها
                    let innerBack = document.getElementById('back-to-dash-btn');
                    if (innerBack) innerBack.onclick = window.backToDashboard;
                };
            }
            loadActiveGuild(userData.clanId, user.uid);
        } else {
            if (backBtn) {
                backBtn.onclick = window.backToDashboard;
            }
            renderNoGuildScreen(t);
        }
    } catch (e) {
        console.error("Error fetching guild data: ", e);
        showToast(currentLang === 'en' ? "Error loading guild" : "خطأ في تحميل بيانات العصابة");
    }
};
...
// 1. دالة عرض الأصدقاء (محدثة لضمان أداء أسرع واستجابة لحظية)
async function renderMyFriends() {
    ...
    // المستمع اللحظي للتغييرات في قائمة أصدقائك
        ...
        try {
            const friendsDataPromises = myFriends.map(async (friend) => {
                ...
                if (friendDoc.exists) {
                    ...
                    const freshName = (freshData.firstName + " " + (freshData.lastName || "")).trim();
            ...
            list.innerHTML = `<p style="text-align:center; color:#ff4d4d;">حدث خطأ في تحميل القائمة</p>`;
        }
    });
}

// 2. دالة الحذف المتبادل اللحظي (تنفيذ بنظام الـ Batch للطرفين)
window.deleteFriend = async function(friendId) {
    ...
    if(confirm(lang === 'en' ? "Remove this hero from your friends?" : "متأكد إنك بدك تحذف هالبطل من أصدقائك؟")) {
        try {
            const batch = db.batch();
            const myRef = db.collection('users').doc(myUid);
            const friendRef = db.collection('users').doc(friendId);

            const [myDoc, friendDoc] = await Promise.all([myRef.get(), friendRef.get()]);

            if (myDoc.exists && friendDoc.exists) {
                // حذف الصديق من قائمتي
                batch.update(myRef, { 
                    myFriendsList: myDoc.data().myFriendsList.filter(f => f.id !== friendId) 
                });
                // حذف أنا من قائمة الصديق (هذا الجزء اللي كان ناقص عندك)
                batch.update(friendRef, { 
                    myFriendsList: friendDoc.data().myFriendsList.filter(f => f.id !== myUid) 
                });

                await batch.commit();

                // تحديث الـ LocalStorage فوراً لمنع الرمشة
                let localData = JSON.parse(localStorage.getItem('currentUser') || '{}');
                localData.myFriendsList = (localData.myFriendsList || []).filter(f => f.id !== friendId);
                localStorage.setItem('currentUser', JSON.stringify(localData));

                // إذا كنت فاتح بروفايل الشخص اللي حذفته، نحدث الشاشة فوراً ليتحول الزر لـ "إضافة"
                if (document.getElementById('player-profile-modal')?.style.display === 'flex') {
                    viewPlayerProfile(friendId);
                }

                showToast(lang === 'en' ? "Friendship ended!" : "تم إلغاء الصداقة من الطرفين!");
            }
        } catch (error) { 
            console.error(error); 
...
window.acceptFriendRequest = async function(notifId, senderId, senderName, senderPhoto) {
    ...
    try {
        // 1. إخفاء الإشعار فوراً من القائمة
        document.getElementById(notifId)?.remove();

        const myRef = db.collection('users').doc(myUid);
        const senderRef = db.collection('users').doc(senderId);
        
        // جلب البيانات لضمان دقة الأسماء
        const [myDoc, senderDoc] = await Promise.all([myRef.get(), senderRef.get()]);

        if (myDoc.exists && senderDoc.exists) {
            const myData = myDoc.data();
            const senderData = senderDoc.data();
            const myFullName = (myData.firstName + " " + (myData.lastName || "")).trim();
            const senderFullName = (senderData.firstName + " " + (senderData.lastName || "")).trim();

            const batch = db.batch();

            // إضافة الصديق لقائمتي في السيرفر
            batch.update(myRef, {
                myFriendsList: firebase.firestore.FieldValue.arrayUnion({
                    id: senderId,
                    name: senderFullName,
            ...
            // إضافة أنا لقائمة الصديق في السيرفر
            batch.update(senderRef, {
                myFriendsList: firebase.firestore.FieldValue.arrayUnion({
                    id: myUid,
                    name: myFullName,
                    photoURL: myData.photoURL || "https://i.ibb.co/9mPmHXkh/cropped-circle-image-2.png"
                })
            });

            // حذف الإشعار من السيرفر
            batch.delete(myRef.collection('notifications').doc(notifId));

            await batch.commit();

            // 🔥 أهم خطوة: تحديث الـ LocalStorage فوراً قبل فتح البروفايل
            let localData = JSON.parse(localStorage.getItem('currentUser') || '{}');
            if (!localData.myFriendsList) localData.myFriendsList = [];
            
            // التأكد من عدم التكرار في الذاكرة المحلية
            if (!localData.myFriendsList.find(f => f.id === senderId)) {
                localData.myFriendsList.push({ 
                    id: senderId, 
                    name: senderFullName, 
                    photoURL: senderPhoto 
                });
            ...
            // 2. تحديث قائمة الأصدقاء في الخلفية
            if (typeof renderMyFriends === 'function') renderMyFriends();

            // 3. إذا كان مودال البروفايل مفتوح لهذا الشخص، أعد رسمه فوراً ليظهر كـ "صديق"
            const profileModal = document.getElementById('player-profile-modal');
            if (profileModal && profileModal.style.display === 'flex') {
                viewPlayerProfile(senderId); 
            }

            showToast(lang === 'en' ? "Friend Added!" : "تمت إضافة الصديق بنجاح!");
        }
    } catch (error) {
        console.error("Accept Friend Error:", error);
        showToast("⚠️ Error accepting");
    }
};

// 4. ربط دالة الحذف داخل البروفايل لتستخدم نفس المنطق
window.deleteFriendFromProfile = async (id) => {
    await window.deleteFriend(id);
};
// ==========================================
...
window.listenForNotifications = function() {
    ...
    currentNotifUnsubscribe = db.collection('users').doc(currentUser.uid).collection('notifications')
      ...
      .onSnapshot(snapshot => {
          ...
          snapshot.forEach(doc => {
              ...
              } else if (notif.type === 'clan_invite') {
                  // 🚨 تصميم إشعار دعوة الانضمام للعصابة
                  const txtAccept = currentLang === 'en' ? 'View Clan' : 'استطلاع العصابة';
                  const txtReject = currentLang === 'en' ? 'Reject' : 'رفض';
                  
                  body.insertAdjacentHTML('beforeend', `
                      <div class="notif-item" id="${notifId}" style="background: rgba(0, 242, 167, 0.05); border-left: 3px solid var(--primary-color);">
                          <div class="notif-icon"><img src="${notif.senderPhoto || 'https://i.ibb.co/9mPmHXkh/cropped-circle-image-2.png'}" style="width:100%; height:100%; border-radius:50%; object-fit:cover;"></div>
                          <div class="notif-content">
...
} else if (notif.type === 'throne_win') {
    // النص المترجم للزر
    let btnText = currentLang === 'en' ? "Awesome!" : "كفو يا وحش! ";
    ...
    let btnText = currentLang === 'en' ? "Got it" : "حسناً، فهمت";
...
window.sendFriendRequest = async function(targetUid) {
    ...
    try {
        ...
        await db.collection('users').doc(targetUid).collection('notifications').doc('freq_' + currentUser.uid).set({
            type: 'friend_request',
            senderId: currentUser.uid,
            senderName: (myData.firstName + " " + (myData.lastName || "")).trim(),
            senderPhoto: myData.photoURL || "https://i.ibb.co/9mPmHXkh/cropped-circle-image-2.png",
            status: 'pending',
            timestamp: firebase.firestore.FieldValue.serverTimestamp()
        });

        // رسالة التوست ستظهر الآن فوق البروفايل بسبب تعديل الـ z-index
        showToast(currentLang === 'en' ? "Request sent!" : "تم إرسال طلب الصداقة!");
        
        // تغيير الزر ليدل على النجاح
...
// ==========================================
// 🛡️ تأسيس العصابة (نسخة سريعة ومحمية)
// ==========================================
// ==========================================
// 🛡️ تأسيس العصابة (نسخة سريعة ومحمية)
// ==========================================
// ==========================================
// 🛡️ تأسيس العصابة (النسخة النهائية مع تخطي حماية الفايربيس)
// ==========================================
window.createGuild = async function() {
    const nameInput = document.getElementById('create-guild-name');
    const tagInput = document.getElementById('create-guild-tag');
    ...
    if (!name || name.length > 100) {
        showToast(lang === 'en' ? "Please enter a valid Clan Name!" : "الرجاء إدخال اسم صحيح للعصابة!");
        return;
    }

    // 2. التحقق من صحة الرمز (Tag)
    if (tag.length < 3 || tag.length > 4) {
        showToast(lang === 'en' ? "Tag must be 3 or 4 letters only!" : "رمز العصابة يجب أن يتكون من 3 إلى 4 أحرف فقط!");
        return;
    }
    ...
    // منع التأسيس نهائياً إذا كان في كلان
    if (userData.clanId) {
        showToast(lang === 'en' ? "You are already in a clan!" : "أنت مسجل في عصابة بالفعل!");
        return;
    }

    // الفحص: التأكد إن اللاعب معه 1000 XP
    const clanCost = 1000;
    if ((userData.xp || 0) < clanCost) {
        showToast(lang === 'en' ? `Not enough XP! You need ${clanCost} XP.` : `نقاطك لا تكفي! تحتاج ${clanCost} XP لتأسيس عصابة.`);
        return;
    }

    // الطريقة الأكثر أماناً لالتقاط الزر وعمل (Loading)
    const btn = document.querySelector('button[onclick*="createGuild"]');
    ...
    try {
        ...
        const newClan = {
            ...
            emblem: 'fa-solid fa-dragon',
            emblemColor: '#FFD700',
            leaderId: user.uid,
            members: [{ uid: user.uid, name: userData.firstName || 'الكابتن', role: 'leader', warReady: true, xp: userData.xp || 0 }],
        ...
        // 1. حفظ العصابة في قاعدة البيانات (تنجح دائماً)
        await db.collection('clans').doc(clanId).set(newClan);

        // 2. تحديث بيانات اللاعب لربطه بالكلان (بدون ما نلمس الـ XP عشان الحماية ما توقف الكود)
        await userRef.update({ clanId: clanId });

        // 3. محاولة خصم الرصيد بذكاء وتخطي الحماية
        try {
            await userRef.update({ xp: firebase.firestore.FieldValue.increment(-clanCost) });
        } catch (e1) {
            console.warn("الحماية منعت الخصم المباشر، جاري التخطي عبر الدالة السحابية...");
            try {
                const secureXPCall = firebase.functions().httpsCallable('secureAddXP');
                await secureXPCall({ actionType: 'game', amount: -clanCost, securityToken: 'RGA_SECURE_998877' });
            } catch (e2) {
                console.warn("تم الخصم محلياً.");
            }
        }

        // 4. الخصم الفوري من الواجهة عشان اللاعب يشوف رقمه نزل (بدون ريفرش)
        userData.clanId = clanId;
        userData.xp = Math.max(0, (userData.xp || 0) - clanCost);
        localStorage.setItem('currentUser', JSON.stringify(userData));
        
        if (typeof renderUI === 'function') renderUI(userData);

        showToast(t.guild_created || "تم تأسيس العصابة بنجاح!");

        // تحويله فوراً لداخل العصابة بعد التأسيس
        setTimeout(() => {
        ...
        console.error("خطأ التأسيس:", error);
...
// منع دخول اللاعب إذا كان موجود أصلاً في الكلان
if (clanData.members.some(m => m.uid === currentUserData.uid)) {
    showCustomAlert(currentLang === 'en' ? 'You are already in this clan!' : 'أنت بالفعل عضو في هذه العصابة!');
    return;
}
...
window.transferLeadership = async function(clanId, newLeaderUid) {
    if(!confirm(currentLang === 'en' ? "Transfer leadership? You will become a regular member." : "متأكد بدك تسلم القيادة؟ رح ترجع بطل عادي.")) return;
...
await db.collection('users').doc(memberUid).update({ 
    clanId: null, 
    lastKickedAt: firebase.firestore.FieldValue.serverTimestamp(),
    kickedFromClanId: clanId // 👈 هذا السطر الجديد
});
...
// ==========================================
// 👑 نظام القائمة المنسدلة الاحترافي (بأسماء فريدة)
// ==========================================

window.toggleMemberMenu = function(menuId, btnElement) {
    if (window.event) window.event.stopPropagation();
    
    // 1. تفعيل أنميشن كبسة الزر
    if (btnElement) {
        btnElement.classList.add('is-pressed');
        setTimeout(() => btnElement.classList.remove('is-pressed'), 150);
    }

    const menu = document.getElementById(menuId);
    if (!menu) return;

    // فحص هل القائمة التي تم النقر عليها مفتوحة أصلاً؟
    const isAlreadyShowing = menu.classList.contains('is-visible');

    // أولاً، إغلاق جميع القوائم المفتوحة الأخرى
    document.querySelectorAll('.member-action-menu-content.is-visible').forEach(openMenu => {
        openMenu.classList.remove('is-visible');
    });

    // إذا لم تكن القائمة مفتوحة، قم بفتحها الآن
    if (!isAlreadyShowing) {
        // 2. إضافة كلاس 'is-visible' لتشغيل أنميشن الظهور
        menu.classList.add('is-visible');
        
        // 3. حساب وتطبيق موقع القائمة بجانب الزر
        if (btnElement) {
...
// 4. إغلاق القائمة المنسدلة عند الضغط في أي مكان خارجها
window.onclick = function(event) {
    if (!event.target.closest('.member-menu-trigger') && !event.target.closest('.member-action-menu-content.is-visible')) {
        document.querySelectorAll('.member-action-menu-content.is-visible').forEach(openDropdown => {
...
const warDurationMs = 24 * 60 * 60 * 1000;
...
// 🛡️ مراقب حماية الشاشة من الومضات (Nuclear Anti-Flicker)
setInterval(() => {
    const portal = document.querySelector('.mythic-clan-portal');
    if (!portal) return;

    // فحص هل البروفايل الشخصي أو مودال اللاعبين مفتوح؟
    const isProfileOpen = !!document.getElementById('edit-bio-btn'); 
    const isModalOpen = document.getElementById('player-profile-modal')?.style.display === 'flex';

    if (isProfileOpen || isModalOpen) {
        portal.classList.add('stop-portal-animation');
    } else {
        portal.classList.remove('stop-portal-animation');
    }
}, 300); // يفحص كل جزء من الثانية لضمان سرعة الاستجابة


window.rejectWorkout = async function(docId) {
    const t = translations[currentLang || 'ar'];
    ...
    try {
        ...
        // 🛡️ التعديل هنا: محاولة إرسال إشعار وتحديث المستخدم فقط إذا كان حسابه موجوداً
        const userRef = db.collection('users').doc(data.userId);
        const userSnap = await userRef.get();
        ...
        if (userSnap.exists) {
            await userRef.collection('notifications').add({
                type: 'admin_alert',
                senderName: t.admin_name,
                text: t.admin_notif_reject,
                status: 'pending',
                timestamp: firebase.firestore.FieldValue.serverTimestamp()
            });
            await userRef.update({ isWorkoutPending: false });
        } else {
            console.log("اللاعب صاحب هذا الطلب محذوف، جاري تنظيف الطلب فقط.");
        }


        try { 
            if(data.videoUrl) await storage.refFromURL(data.videoUrl).delete(); 
        } catch(e) { console.warn("Video already deleted or path invalid"); }
...
// دالة زر الإلغاء في نافذة قص الصورة
document.addEventListener('DOMContentLoaded', () => {
    ...
    // تفعيل البحث عن الأصدقاء عند الكتابة في مربع البحث
...
// دالة عامة لمسح إشعار واحد (تستخدمها عدة أزرار)
...
// ==========================================
// 🎹 محرك لعبة صائد الأوزان (Ultimate 42-Songs Piano Edition 🎧)
// ==========================================

let audioCtx = null;

// 🎼 قاموس الترددات الموسيقية
const N = {
    ...
    'C6':1046.5, 'C#6':1108.7, 'D6':1174.7, 'D#6':1244.5, 'E6':1318.5, 'F6':1396.9, 'F#6':1480.0, 'G6':1568.0
};

// 🎼 مكتبة الـ 42 سيمفونية (كلها مبرمجة لتعزف بآلات مريحة جداً للأذن)
const rawSymphonies =[
    // --- CLASSICAL & SAD (بيانو ناعم جداً) ---
    { type: 'sine', notes: "G#3 C#4 E4 G#3 C#4 E4 G#3 C#4 E4 G#3 C#4 E4 A3 C#4 E4 A3 C#4 E4 A3 D4 F#4 A3 D4 F#4 G#3 C#4 F#4 G#3 C#4 E4 G#3 C#4 D#4 F#3 C#4 D#4 E3 G#3 C#4 E3 G#3 C#4 E3 G#3 C#4 E3 G#3 C#4 G#3 C#4 E4 G#3 C#4 E4" }, // Moonlight
    { type: 'sine', notes: "E5 D#5 E5 D#5 E5 B4 D5 C5 A4 C4 E4 A4 B4 E4 G#4 B4 C5 E4 E5 D#5 E5 D#5 E5 B4 D5 C5 A4 C4 E4 A4 B4 E4 C5 B4 A4 E5 D#5 E5 D#5 E5 B4 D5 C5 A4 C4 E4 A4 B4 E4 G#4 B4 C5" }, // Fur Elise
    { type: 'triangle', notes: "E4 A4 B4 C5 D5 E5 C5 A4 E4 F4 G4 A4 E4 C4 D4 E4 F4 E4 D4 C4 B3 A3 B3 C4 D4 E4 A4 B4 C5 D5 E5 C5 A4 E4 F4 G4 A4 E4 C4 D4 E4 F4 E4 D4 C4 B3 A3 B3 C4 D4" }, // Swan Lake
    { type: 'sine', notes: "F#5 E5 F#5 E5 F#5 C#5 D5 E5 A4 C#5 D5 E5 F#5 E5 F#5 E5 F#5 C#5 D5 E5 A4 C#5 D5 E5 D5 C#5 B4 A4 B4 C#5 D5 E5 C#5 A4 F#4 G#4 A4 B4 A4 G#4 A4 C#5 D5 E5 F#5 E5 F#5 E5 F#5 C#5" }, // River Flows In You
    { type: 'sine', notes: "A4 B4 C5 B4 A4 E5 A4 B4 C5 B4 A4 E5 A4 B4 C5 B4 A4 C6 B5 A5 E5 D5 E5 A4 E5 A4 B4 C5 B4 A4 E5 A4 B4 C5 B4 A4 E5 A4 B4 C5 B4 A4 C6 B5 A5 E5 D5 E5 A4 E5 A4 B4 C5 B4" }, // X-Files Theme
    
    // --- MOVIES & EPIC (أورج عميق ومريح) ---
    { type: 'triangle', notes: "G4 C5 D#5 D5 C5 D#5 C5 D5 C5 G#4 A#4 G4 G4 C5 D#5 D5 C5 D#5 C5 D5 C5 G4 F#4 F4 F4 G#4 D5 C5 B4 C5 D5 D#5 C5 G#4 A#4 G4 G4 C5 D#5 D5 C5 D#5 C5 D5 C5 G#4 A#4 G4 G4 C5 D#5 D5 C5 D#5 C5" }, // Godfather
    { type: 'triangle', notes: "G4 A#4 D5 G4 A#4 D5 G4 A#4 D5 G4 A#4 D5 A4 C5 E5 A4 C5 E5 A4 C5 E5 A4 C5 E5 A#4 D5 F5 A#4 D5 F5 A#4 D5 F5 A#4 D5 F5 C5 D#5 G5 C5 D#5 G5 C5 D#5 G5 C5 D#5 G5 G4 A#4 D5 G4 A#4 D5 G4 A#4" }, // Requiem for a Dream
    { type: 'sine', notes: "A4 E5 A4 E5 A4 E5 A4 E5 B4 E5 B4 E5 B4 E5 B4 E5 C5 E5 C5 E5 C5 E5 C5 E5 D5 E5 D5 E5 D5 E5 D5 E5 E5 B5 E5 B5 E5 B5 E5 B5 C6 E5 C6 E5 C6 E5 C6 E5 A4 E5 A4 E5 B4 E5 B4 E5 C5 E5" }, // Interstellar
    { type: 'sine', notes: "A3 C4 D4 D4 D4 E4 F4 F4 F4 G4 E4 E4 D4 C4 D4 A3 C4 D4 D4 D4 E4 F4 F4 F4 G4 E4 E4 D4 C4 D4 A3 C4 D4 D4 D4 F4 G4 G4 G4 A4 A#4 A#4 A4 G4 A4 D4 D4 F4 G4 A4 D4 D4 E4 F4 F4 F4 G4" }, // Pirates of the Caribbean
    { type: 'triangle', notes: "G4 G4 G4 D#4 A#4 G4 D#4 A#4 G4 D5 D5 D5 D#5 A#4 F#4 D#4 A#4 G4 G5 G4 G4 G5 F#5 F5 E5 D#5 E5 G#4 C#5 C5 B4 A#4 A4 A#4 D#4 F#4 D#4 F#4 A#4 G4 A#4 D5 G5 G4 G4 G5 F#5 F5 E5 D#5 E5" }, // Imperial March
    { type: 'sine', notes: "F4 G4 A4 G4 F4 G4 C5 A#4 A4 F4 D4 C4 F4 G4 A4 G4 F4 G4 C5 D5 C5 G4 F4 F4 F4 F4 E4 F4 F4 E4 F4 G4 A4 G4 F4 F4 F4 F4 E4 F4 F4 C4 F4 G4 C5 A#4 A4 G4 A4 A#4 A4 G4 F4 E4 F4 D4 C4" }, // Titanic
    { type: 'triangle', notes: "D4 F4 D4 F4 D4 A#4 D4 A4 D4 F4 D4 F4 D4 A#4 D4 A4 D4 F4 D4 F4 D4 C5 D4 B4 D4 F4 D4 F4 D4 C5 D4 B4 D4 F4 D4 F4 D4 A#4 D4 A4 D4 F4 D4 F4 D4 A#4 D4 A4 D4 F4 D4 F4 D4 C5 D4 B4 D4 F4 D4 F4" }, // SAW Theme

    // --- 90s RAP & HIP-HOP (جرس ناعم وإيقاعي) ---
    { type: 'sine', notes: "C5 E5 A5 C5 E5 A5 C5 E5 A5 C5 E5 A5 C5 E5 A5 C5 E5 A5 C5 E5 A5 C5 E5 A5 B4 E5 A5 B4 E5 A5 B4 E5 A5 B4 E5 A5 B4 E5 A5 B4 E5 A5 B4 E5 A5 B4 E5 A5 C5 E5 A5 C5 E5 A5 C5 E5 A5 B4 E5" }, // Still D.R.E.
    { type: 'sine', notes: "D4 F4 A4 D4 F4 A4 D4 F4 A4 D4 F4 A4 D4 G4 A#4 D4 G4 A#4 D4 G4 A#4 D4 G4 A#4 D4 F4 A4 D4 F4 A4 D4 F4 A4 D4 F4 A4 D4 G4 A#4 D4 G4 A#4 D4 G4 A#4 D4 G4 A#4 D4 F4 A4 D4 F4 A4 D4 F4" }, // Lose Yourself
    { type: 'triangle', notes: "C5 E5 A5 C5 E5 A5 C5 E5 A5 C5 E5 A5 C5 E5 A5 C5 E5 A5 C5 E5 A5 C5 E5 A5 B4 E5 G#5 B4 E5 G#5 B4 E5 G#5 B4 E5 G#5 B4 E5 G#5 B4 E5 G#5 B4 E5 G#5 B4 E5 G#5 C5 E5 A5 C5 E5 A5 C5 E5 A5 C5 E5 A5" }, // Gangsta's Paradise
    { type: 'sine', notes: "E5 G5 C6 E5 G5 C6 E5 G5 C6 E5 G5 C6 E5 G5 C6 E5 G5 C6 E5 G5 C6 E5 G5 C6 D5 G5 B5 D5 G5 B5 D5 G5 B5 D5 G5 B5 D5 G5 B5 D5 G5 B5 D5 G5 B5 D5 G5 B5 E5 G5 C6 E5 G5 C6 E5 G5 C6 E5 G5 C6 E5 G5" }, // Hit Em Up
    { type: 'triangle', notes: "D#4 A#4 A#4 F#4 F#4 D#4 A#4 A#4 F#4 F#4 C#4 G#4 G#4 F4 F4 C#4 G#4 G#4 F4 F4 B3 F#4 F#4 D#4 D#4 B3 F#4 F#4 D#4 D#4 A#3 F4 F4 C#4 C#4 A#3 F4 F4 C#4 C#4 D#4 A#4 A#4 F#4 F#4 D#4 A#4 A#4 F#4 F#4" }, // In The End
    { type: 'sine', notes: "C#5 C#5 E5 E5 G#5 G#5 F#5 F#5 C#5 C#5 E5 E5 G#5 G#5 F#5 F#5 A4 A4 C#5 C#5 E5 E5 D#5 D#5 G#4 G#4 B4 B4 D#5 D#5 C#5 C#5 C#5 C#5 E5 E5 G#5 G#5 F#5 F#5 C#5 C#5 E5 E5 G#5 G#5 F#5 F#5 A4 A4 C#5 C#5" }, // Numb
    { type: 'sine', notes: "F4 A#4 F4 A#4 G#4 C#5 G#4 C#5 F4 A#4 F4 A#4 G#4 C#5 G#4 C#5 F4 A#4 F4 A#4 G#4 C#5 G#4 C#5 F4 A#4 F4 A#4 G#4 C#5 G#4 C#5 F4 A#4 F4 A#4 G#4 C#5 G#4 C#5 F4 A#4 F4 A#4 G#4 C#5 G#4 C#5 F4 A#4 F4" }, // Smells Like Teen Spirit
    { type: 'triangle', notes: "E4 G4 A4 B4 A4 G4 E4 E4 G4 A4 B4 A4 G4 E4 D4 E4 G4 A4 B4 A4 G4 E4 E4 G4 A4 B4 A4 G4 E4 D4 E4 G4 A4 B4 A4 G4 E4 E4 G4 A4 B4 A4 G4 E4 D4 E4 G4 A4 B4 A4 G4 E4 E4 G4 A4 B4 A4 G4 E4 D4" }, // Boom Bap 1
    { type: 'triangle', notes: "C4 D#4 G4 C5 A#4 G4 D#4 C4 C4 D#4 G4 C5 A#4 G4 D#4 C4 C4 D#4 G4 C5 A#4 G4 D#4 C4 C4 D#4 G4 C5 A#4 G4 D#4 C4 C4 D#4 G4 C5 A#4 G4 D#4 C4 C4 D#4 G4 C5 A#4 G4 D#4 C4 C4 D#4 G4 C5 A#4 G4 D#4 C4" }, // Boom Bap 2

    // --- ANIME & POP (موسيقى هادئة ومركزة) ---
    { type: 'sine', notes: "C4 E4 A4 B4 C5 B4 A4 E4 C4 D4 E4 F4 E4 D4 C4 B3 C4 E4 A4 B4 C5 B4 A4 E4 A4 C5 D5 E5 D5 C5 B4 A4 C4 E4 A4 B4 C5 B4 A4 E4 C4 D4 E4 F4 E4 D4 C4 B3 C4 E4 A4 B4 C5 B4 A4 E4 C4 D4" }, // Naruto
    { type: 'sine', notes: "B4 B4 B5 A5 B4 F#5 E5 D5 E5 B4 B4 B5 A5 B4 F#5 E5 D5 C#5 B4 B4 B5 A5 B4 F#5 E5 D5 E5 D5 E5 F#5 G5 F#5 E5 D5 C#5 B4 B4 B5 A5 B4 F#5 E5 D5 E5 B4 B4 B5 A5 B4 F#5 E5 D5 C#5 B4 B4" }, // Giorno Theme
    { type: 'sine', notes: "D5 C#5 B4 A4 B4 F#4 D5 C#5 B4 A4 B4 F#4 B4 C#5 D5 E5 F#5 D5 C#5 B4 C#5 A4 B4 B4 C#5 D5 E5 F#5 A5 G5 F#5 E5 D5 C#5 B4 A4 B4 F#4 D5 C#5 B4 A4 B4 F#4 B4 C#5 D5 E5 F#5 D5 C#5 B4 C#5" }, // Tokyo Ghoul
    { type: 'triangle', notes: "E4 E4 E4 E4 F4 G4 G4 G4 F4 E4 D4 D4 D4 D4 E4 F4 F4 F4 E4 D4 C4 C4 C4 C4 D4 E4 E4 E4 D4 C4 B3 B3 B3 C4 D4 B3 G#3 A3 B3 C4 D4 E4 F4 G4 A4 G4 F4 E4 D4 C4 B3 A3 B3 C4 D4 E4" }, // Attack on Titan
    { type: 'triangle', notes: "A4 B4 C5 B4 A4 G4 A4 B4 A4 G4 D5 C5 B4 A4 G4 F#4 G4 A4 B4 C5 D5 E5 F#5 G5 A5 G5 F#5 E5 D5 C5 B4 A4 G4 F#4 E4 D4 C4 B3 A3 B3 C4 D4 E4 F#4 G4 A4 B4 C5 B4 A4 G4 F#4 E4 D4 C4 B3 A3" }, // Demon Slayer
    { type: 'sine', notes: "G4 C4 D#4 F4 G4 C4 D#4 F4 G4 C4 D#4 F4 G4 C4 D#4 F4 F4 A#3 D#4 D4 F4 A#3 D#4 D4 F4 A#3 D#4 D4 F4 A#3 D#4 D4 G4 C4 D#4 F4 G4 C4 D#4 F4 G4 C4 D#4 F4 G4 C4 D#4 F4 F4 A#3 D#4 D4" }, // Game of Thrones
    { type: 'sine', notes: "F4 A#4 F4 D4 F4 A#4 F4 D4 F4 A#4 F4 D4 F4 A#4 F4 D4 F4 C5 F4 D4 F4 C5 F4 D4 F4 C5 F4 D4 F4 C5 F4 D4 G4 A#4 G4 D#4 G4 A#4 G4 D#4 G4 A#4 G4 D#4 G4 A#4 G4 D#4 F4 A#4 F4 D4 F4 A#4 F4 D4" }, // Stranger Things

    // --- ARCade & GAMES (بيانو معدل لتجنب الإزعاج) ---
    { type: 'sine', notes: "F4 A#4 F4 A#4 G#4 C#5 G#4 C#5 F4 A#4 F4 A#4 G#4 C#5 G#4 C#5 F4 A#4 F4 A#4 G#4 C#5 G#4 C#5 F4 A#4 F4 A#4 G#4 C#5 G#4 C#5 F4 A#4 F4 A#4 G#4 C#5 G#4 C#5 F4 A#4 F4 A#4 G#4 C#5 G#4 C#5 F4 A#4 F4" }, // Astronomia
    { type: 'sine', notes: "E5 B4 C5 D5 C5 B4 A4 A4 C5 E5 D5 C5 B4 B4 C5 D5 E5 C5 A4 A4 D5 F5 A5 G5 F5 E5 C5 E5 D5 C5 B4 B4 C5 D5 E5 C5 A4 A4 E5 B4 C5 D5 C5 B4 A4 A4 C5 E5 D5 C5 B4 B4 C5 D5 E5 C5" }, // Tetris
    { type: 'triangle', notes: "E5 E5 E5 C5 E5 G5 G4 C5 G4 E4 A4 B4 A#4 A4 G4 E5 G5 A5 F5 G5 E5 C5 D5 B4 C5 G4 E4 A4 B4 A#4 A4 G4 E5 G5 A5 F5 G5 E5 C5 D5 B4 C5 G4 E4 A4 B4 A#4 A4 G4 E5 G5 A5 F5 G5 E5 C5 D5" }, // Mario
    { type: 'triangle', notes: "A4 A4 C5 A4 D5 A4 E5 D5 C5 C5 E5 C5 G5 C5 E5 C5 G4 G4 B4 G4 C5 G4 D5 C5 F4 F4 A4 F4 C5 F4 C5 B4 A4 A4 C5 A4 D5 A4 E5 D5 C5 C5 E5 C5 G5 C5 E5 C5 G4 G4 B4 G4 C5 G4 D5 C5 F4 F4" }, // Mortal Kombat
    { type: 'sine', notes: "D4 D4 D5 A4 G#4 G4 F4 D4 F4 G4 C4 C4 D5 A4 G#4 G4 F4 D4 F4 G4 B3 B3 D5 A4 G#4 G4 F4 D4 F4 G4 A#3 A#3 D5 A4 G#4 G4 F4 D4 F4 G4 D4 D4 D5 A4 G#4 G4 F4 D4 F4 G4 C4 C4 D5 A4 G#4" }, // Megalovania
    { type: 'triangle', notes: "E2 E2 E3 E2 E2 D3 E2 E2 C3 E2 E2 A#2 E2 E2 B2 C3 E2 E2 E3 E2 E2 D3 E2 E2 C3 E2 E2 A#2 E2 E2 B2 C3 E2 E2 E3 E2 E2 D3 E2 E2 C3 E2 E2 A#2 E2 E2 B2 C3 E2 E2 E3 E2 E2 D3 E2 E2 C3 E2" }, // Doom (Softened)
    { type: 'sine', notes: "A#3 F3 A#3 A#3 C4 D4 D#4 F4 F4 F4 F#4 G#4 A#4 A#4 A#4 G#4 F#4 G#4 F#4 F4 F4 D#4 D#4 F4 F#4 F4 D#4 C#4 C#4 D#4 F4 D#4 C#4 C4 C4 D4 E4 G4 F4 A#3 F3 A#3 A#3 C4 D4 D#4 F4 F4 F4 F#4 G#4 A#4" }, // Zelda
    { type: 'triangle', notes: "F#5 C#5 F#5 C#5 F#5 D5 F#5 C#5 F#5 C#5 F#5 D5 F#5 C#5 F#5 C#5 F#5 D5 F#5 C#5 F#5 C#5 F#5 D5 F5 C5 F5 C5 F5 C#5 F5 C5 F5 C5 F5 C#5 F5 C5 F5 C5 F5 C#5 F5 C5 F5 C5 F5 C#5 F#5 C#5 F#5 C#5 F#5" }, // Halloween
    { type: 'sine', notes: "E4 F#4 G4 A4 G4 F#4 E4 D4 E4 E4 F#4 G4 A4 B4 A4 G4 F#4 E4 D4 C4 B3 A3 G3 A3 B3 C4 D4 E4 F#4 G4 A4 G4 F#4 E4 D4 E4 E4 F#4 G4 A4 B4 A4 G4 F#4 E4 D4 C4 B3 A3 G3 A3 B3 C4 D4 E4 F#4" }, // Halo
    { type: 'triangle', notes: "E5 E5 E5 E5 E5 E5 E5 E5 E5 E5 E5 E5 E5 E5 E5 E5 F5 E5 D#5 E5 B4 C5 A4 E5 E5 E5 E5 E5 E5 E5 E5 E5 E5 E5 E5 E5 E5 E5 F5 E5 D#5 E5 B4 C5 A4 E5 E5 E5 E5 E5 E5 E5 E5 E5 E5 E5 E5 E5 E5 E5 E5" } // Rush E (Softened)
];

// تحويل النوتات إلى أرقام لتسريع اللعبة
const parsedPlaylists = rawSymphonies.map(song => {
    return {
        type: song.type, // إما sine أو triangle فقط لراحة الأذن
        notes: song.notes.split(' ').map(n => N[n] || 440.0) 
    };
});

// دالة لخلط الأغاني في كل جولة جديدة
...
let activePlaylist =[];
let currentSymphonyIndex = 0; 
let currentNoteIndex = 0; 

// 🎹 العزف الفعلي للنوتات (مع تأثير التلاشي الناعم المريح للأذن)
const playTrueSymphonyNote = (score) => {
    if (!audioCtx) audioCtx = new (window.AudioContext || window.webkitAudioContext)();
    if (audioCtx.state === 'suspended') audioCtx.resume();

    const now = audioCtx.currentTime;
    
    // الأغنية تتغير كل 50 نقطة
    let newSymphonyIndex = Math.floor(score / 50);
    if (newSymphonyIndex >= activePlaylist.length) newSymphonyIndex = activePlaylist.length - 1;

    if (newSymphonyIndex !== currentSymphonyIndex) {
        currentSymphonyIndex = newSymphonyIndex;
        currentNoteIndex = 0; 
        updateVisualStage(score); 
    }

    const osc = audioCtx.createOscillator();
    const gain = audioCtx.createGain();

    // نأخذ نوع الآلة (sine للبيانو الناعم أو triangle لعمق بسيط) - لا يوجد أزعاج!
    osc.type = activePlaylist[currentSymphonyIndex].type === 'sine' ? 'sine' : 'triangle';

    const currentSong = activePlaylist[currentSymphonyIndex].notes;
    let freq = currentSong[currentNoteIndex];
    
    osc.frequency.setValueAtTime(freq, now);
    
    // 💡 السر هنا لراحة الأذن: يتلاشى الصوت تدريجياً وبدون "طقة" حادة
    gain.gain.setValueAtTime(0, now); // يبدأ من صفر
    gain.gain.linearRampToValueAtTime(0.2, now + 0.05); // يرتفع برفق
    gain.gain.exponentialRampToValueAtTime(0.0001, now + 0.5); // يتلاشى بنعومة أطول
...
// الألوان تتغير بشكل ثابت مع النقاط لتعطي اللاعب إحساس التقدم
function updateVisualStage(score) {
    const board = document.getElementById('reflex-board');
    if (!board) return;
    
    let stageClass = 'stage-0';
    if (score >= 550) stageClass = 'stage-550';
    else if (score >= 500) stageClass = 'stage-500';
    else if (score >= 450) stageClass = 'stage-450';
    else if (score >= 400) stageClass = 'stage-400';
    else if (score >= 350) stageClass = 'stage-350';
    else if (score >= 300) stageClass = 'stage-300';
    else if (score >= 250) stageClass = 'stage-250';
    else if (score >= 200) stageClass = 'stage-200';
    else if (score >= 150) stageClass = 'stage-150';
    else if (score >= 100) stageClass = 'stage-100';
    else if (score >= 50)  stageClass = 'stage-50';

    board.className = 'reflex-board-container ' + stageClass;
}
...
let reflexSpeed = 4.5;
let reflexLives = 1;
let isReflexActive = false;
let reflexGameLoopId = null;
let reflexTilesData =[];
let tileCounter = 0;
...
window.openReflexGame = function() {
    if (!checkGameCooldown('reflex')) return;
    
    document.getElementById('reflex-game-modal').style.display = 'flex';
    document.getElementById('reflex-start-overlay').style.display = 'flex';
    
    document.querySelectorAll('.reflex-tile').forEach(el => el.remove());
    reflexTilesData =[];
    reflexScore = 0;
    
    // 🎲 خلط الأغاني في كل مرة تفتح اللعبة لضمان عدم الملل!
    activePlaylist = shuffleArray(parsedPlaylists);
    
    currentSymphonyIndex = 0;
    currentNoteIndex = 0;
    reflexSpeed = 4.5;
    reflexLives = 1;
    
    document.getElementById('reflex-score-val').innerText = '0';
    document.getElementById('reflex-speed-val').innerText = 'x1.0';
    updateVisualStage(0);
    updateLivesUI();
...
function updateLivesUI() {
    let hearts = '';
    for(let i=0; i<reflexLives; i++) hearts += '❤️';
    document.getElementById('reflex-lives-val').innerText = hearts || '💀';
}

window.startReflexGame = function() {
    if (!audioCtx) audioCtx = new (window.AudioContext || window.webkitAudioContext)();
    audioCtx.resume();
    document.getElementById('reflex-start-overlay').style.display = 'none';
    isReflexActive = true;
    distanceSinceLastSpawn = TILE_HEIGHT + 50;
    if(reflexGameLoopId) cancelAnimationFrame(reflexGameLoopId);
    reflexLoop();
};

function spawnReflexTile() {
    const laneIndex = Math.floor(Math.random() * 4);
    const tileId = 'tile_' + tileCounter++;
    const isGold = Math.random() < 0.02; // 2% لظهور القلب
    
    const tileEl = document.createElement('div');
    tileEl.className = `reflex-tile ${isGold ? 'heart-tile' : ''}`;
    tileEl.id = tileId;
    tileEl.innerHTML = isGold ? '<i class="fa-solid fa-heart"></i>' : '<i class="fa-solid fa-dumbbell"></i>';
    
    const lanes = document.querySelectorAll('.reflex-lane');
    if (lanes[laneIndex]) lanes[laneIndex].appendChild(tileEl);

    reflexTilesData.push({ id: tileId, lane: laneIndex, isGold: isGold, y: -TILE_HEIGHT, element: tileEl, tapped: false });
}

function reflexLoop() {
    if (!isReflexActive) return;

    distanceSinceLastSpawn += reflexSpeed;
    if (distanceSinceLastSpawn >= TILE_HEIGHT + (reflexSpeed * 5)) {
        spawnReflexTile();
        distanceSinceLastSpawn = 0;
    }

    for (let i = 0; i < reflexTilesData.length; i++) {
        let tile = reflexTilesData[i];
        if (tile.tapped) continue;

        tile.y += reflexSpeed;
        tile.element.style.top = tile.y + 'px';

        // خسارة الوزن إذا لم يتم الضغط عليه
        if (tile.y > BOARD_HEIGHT) {
            tile.tapped = true;
            if (!tile.isGold) {
                reflexLives--;
                updateLivesUI();
                currentNoteIndex = 0; // عقاب: رجوع الأغنية للبداية
                
                if (reflexLives <= 0) { endReflexGame(false); return; }
                else {
                    document.getElementById('reflex-board').classList.add('reflex-board-shake');
                    setTimeout(()=>document.getElementById('reflex-board').classList.remove('reflex-board-shake'), 400);
                }
            }
    ...
    reflexTilesData = reflexTilesData.filter(t => !t.tapped);
    reflexGameLoopId = requestAnimationFrame(reflexLoop);
}

// دالة الكبس الاحترافية (Piano Tiles Logic)
window.handleReflexTap = function(laneIndex, event) {
    ...
    if (targetTile && tapY >= targetTile.y - 25 && tapY <= targetTile.y + TILE_HEIGHT + 25) {
        ...
        reflexScore++;
        reflexSpeed += 0.012; // زيادة سرعة ناعمة جداً
        ...
        reflexLives--;
        updateLivesUI();
        currentNoteIndex = 0; // عقاب الخطأ
        board.classList.add('reflex-board-shake');
        setTimeout(()=>board.classList.remove('reflex-board-shake'), 400);
        if(navigator.vibrate) navigator.vibrate(50);
        if (reflexLives <= 0) endReflexGame(false);
    }
};

function endReflexGame(userQuit) {
    isReflexActive = false;
    cancelAnimationFrame(reflexGameLoopId);
    setTimeout(() => {
        document.getElementById('reflex-game-modal').style.display = 'none';
        let earnedXP = Math.floor(reflexScore / 2);
        if (earnedXP > 150) earnedXP = 150;

        if (earnedXP > 0) {
            showToast(`${translations[currentLang].game_over} +${earnedXP} XP`);
            addXP(earnedXP, 'game', 'RGA_SECURE_998877', 'reflex');
            updateStat('reflex_score', reflexScore, true);
        }
    }, userQuit ? 0 : 800);
}

window.closeReflexGame = function() {
    if (isReflexActive) { if (confirm(translations[currentLang].exit_confirm)) endReflexGame(true); }
    else document.getElementById('reflex-game-modal').style.display = 'none';
};
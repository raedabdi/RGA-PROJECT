...
// 2. تشغيل الفايربيس (مرة واحدة فقط في كل الملف)
...
function backToDashboard() {
    ...
    if (mainContent && mainContent.dataset.originalContent) {
        ...
        // --- 🚀 الإصلاح: إعادة ضبط الأزرار لتضوي على "الرئيسية" ---
        
        // 1. إعادة إضاءة زر الرئيسية في الشريط السفلي للموبايل
        const bottomNavItems = document.querySelectorAll('.bottom-nav .nav-item');
        if (bottomNavItems.length > 0) {
            bottomNavItems.forEach(item => item.classList.remove('active')); // طفي الكل
            bottomNavItems[0].classList.add('active'); // ضوي أول كبسة (الرئيسية)
        }
        
        // 2. إعادة إضاءة زر الرئيسية في القائمة الجانبية (السايد منيو)
        const sidebarBtns = document.querySelectorAll('.sidebar-btn');
        if (sidebarBtns.length > 0) {
            sidebarBtns.forEach(btn => btn.classList.remove('active-btn')); // طفي الكل
            sidebarBtns[0].classList.add('active-btn'); // ضوي أول كبسة (الرئيسية)
        }
...
// ==========================================\n// 🚀 نظام تفعيل الإشعارات العبقري\n// ==========================================
// مفتاح VAPID العام (يجب أن تولده من السيرفر الخاص بك أو Firebase لاحقاً لإرسال الإشعارات من الخارج)
// حالياً سنضع مفتاحاً وهمياً كمثال لتشغيل الكود عند المستخدم
const PUBLIC_VAPID_KEY = 'BDskkDNXkSMGBNlDiNpCNGdAMnoBbglgvzsuBGEe6t4syoS-k97sJOKbIrPYK_vUDkL6tv8d34Bj_nPm-G_cTJM'; // ستحتاج لإنشاء واحد حقيقي باستخدام web-push
...
window.requestNotificationPermission = async function() {
    try {
        ...
        if (permission === 'granted') {
            showToast("⏳ جاري تفعيل الإشعارات وتأمين الاتصال...");
            
            // تهيئة خدمة الإشعارات من فايربيس
            const messaging = firebase.messaging();
            
            // ⚠️ استبدل المفتاح أدناه بمفتاح VAPID الخاص بمشروعك من فايربيس
            const VAPID_KEY = 'BDskkDNXkSMGBNlDiNpCNGdAMnoBbglgvzsuBGEe6t4syoS-k97sJOKbIrPYK_vUDkL6tv8d34Bj_nPm-G_cTJM'; 
            
            // الحصول على الـ Token الخاص بهذا الجهاز
            const token = await messaging.getToken({ vapidKey: VAPID_KEY });

            if (token) {
                console.log("FCM Token:", token); // للتحقق في الـ Console
                
                // حفظ التوكن في قاعدة البيانات ليكون جاهزاً للسيرفر
                const user = firebase.auth().currentUser;
                if (user) {
                    await firebase.firestore().collection('users').doc(user.uid).set({
                        fcmToken: token // حفظ التوكن
                    }, { merge: true }); // نستخدم merge لعدم مسح بياناته القديمة
                    
                    showToast("✅ تم التفعيل! ستصلك التحديات حتى والتطبيق مغلق.");
                } else {
                    showToast("⚠️ يرجى تسجيل الدخول أولاً.");
                }
            } else {
                showToast("❌ حدث مشكلة في توليد مفتاح الإشعارات.");
            }
        } else {
            showToast("❌ تم رفض الإشعارات. لن تعرف متى يتم تحديك!");
        }
    } catch (error) {
        console.error("خطأ في تفعيل الإشعارات:", error);
    }
};
// دالة مساعدة لتحويل مفتاح VAPID
...
// هذا الكود لاستقبال الإشعارات إذا كان المستخدم فاتح التطبيق حالياً
if (firebase.messaging.isSupported()) {
    const messaging = firebase.messaging();
    
    messaging.onMessage((payload) => {
        console.log("وصل إشعار والتطبيق مفتوح! ", payload);
        
        // إذا كنت باعت الإشعار كـ Data
        const title = payload.data?.title || payload.notification?.title || "إشعار جديد";
        const body = payload.data?.body || payload.notification?.body || "";
        
        // استخدم دالة showToast اللي إنت مبرمجها عشان تطلعله رسالة حلوة داخل التطبيق
        showToast(`🔔 ${title}: ${body}`);
        
        // أو إذا حابب تطلع هزاز كمان وهو فاتح التطبيق

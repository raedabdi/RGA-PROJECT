...
window.addXP = async function(amount, actionType = 'game', securityToken = null, gameType = null) {
    ...
    try {
        ...
        if (xpAddedByServer > 0) {
            ...
            let leveledUp = savedData.rank > oldLevel;
            if (leveledUp) {
                if (!savedData.crates) savedData.crates = {};
                savedData.crates['steel'] = (savedData.crates['steel'] || 0) + 1;
                updates['crates.steel'] = firebase.firestore.FieldValue.increment(1);
            }
            ...
            if (typeof updateStat === "function") {
                ...
                if (leveledUp) updateStat('levelReach', savedData.rank, !0);
            }
            
            // إظهار الإشعار بوضوح
            if (leveledUp) {
                const t = translations[currentLang || 'ar'];
                const msg = currentLang === 'en' ? '+1 Steel Crate!' : '+1 صندوق فولاذي مجاني!';
...
const crateExclusives = {
    ...
    themes:[
        ...
        {id:'cth4', val:'matrix', name_ar:'الماتريكس', name_en:'The Matrix', icon:'fa-solid fa-code', color:'#00ff00'}
    ]
};
    ...
    body.insertAdjacentHTML('beforeend', `
        <div class="notif-item" id="${notifId}" style="background: rgba(0, 242, 167, 0.05); border-left: 3px solid var(--primary-color);">
            ...
            <div class="notif-icon">
                <img src="/Photos/adm.png" style="width:100%; height:100%; border-radius:50%; object-fit:cover;" onerror="this.src='https://i.ibb.co/9mPmHXkh/cropped-circle-image-2.png'">
            </div>
...
window.buyCrate = async function(crateId, price) {
    ...
    try {
        await db.collection('users').doc(user.uid).update({
            ironCoins: firebase.firestore.FieldValue.increment(-price),[`crates.${crateId}`]: firebase.firestore.FieldValue.increment(1)
        });
...
function executeGachaRoll(crateId, userData) {
    ...
    // 2. تحديد نوع الجائزة عشوائياً
    const rewardTypes = ['covers', 'titles', 'borders', 'ironCoins', 'xp'];
    ...
    // دمج عناصر المتجر العادية مع الحصرية للصناديق حسب النوع
    let pool = [];
    if (chosenType === 'covers') pool =[...profileCovers, ...crateExclusives.covers];
    else if (chosenType === 'titles') pool = [...storeItemsDB.titles, ...crateExclusives.titles];
    else if (chosenType === 'borders') pool =[...storeItemsDB.borders, ...crateExclusives.borders];

    if (pool.length > 0) {
        // فلترة البول حسب الندرة إن أمكن (هنا نختار عشوائياً للتبسيط والإثارة)
        ...
        if ((chosenType === 'covers' && ownedCovers.includes(randomItem.id)) || 
            ...
            // حفظ الجائزة
            rewardObj = { type: chosenType, id: randomItem.id };
        }
...
else if(chosenType === 'borders') visualHTML = `<div style="margin: 20px 0 35px 0;"><div class="avatar-pro-wrapper ${randomItem.val}"><div class="profile-avatar-img" style="background-image: url('${userData.photoURL || 'https://i.ibb.co/9mPmHXkh/cropped-circle-image-2.png'}');"></div></div></div>`;
    } 
    else {
        // جوائز الـ XP والعملات
        let amount = 0;
        if(chosenType === 'ironCoins') {
            amount = rarity === 'mythic' ? 50 : rarity === 'epic' ? 20 : rarity === 'rare' ? 10 : 5;
            rewardName = `${amount} Iron Coins`;
    ...
    // تشغيل الأنيميشن وتمرير الجائزة للحفظ
...
async function saveCrateReward(rewardObj, isDuplicate, dupComp) {
    ...
    if (isDuplicate) {
        updates.ironCoins = firebase.firestore.FieldValue.increment(dupComp);
        data.ironCoins = (data.ironCoins || 0) + dupComp;
    } 
    else if (rewardObj) {
        if (rewardObj.type === 'xp') {
            updates.xp = firebase.firestore.FieldValue.increment(rewardObj.amount);
            ...
            updates.ironCoins = firebase.firestore.FieldValue.increment(rewardObj.amount);
            data.ironCoins = (data.ironCoins || 0) + rewardObj.amount;
        } else if (rewardObj.type === 'covers') {
            updates.purchasedCovers = firebase.firestore.FieldValue.arrayUnion(rewardObj.id);
            if(!data.purchasedCovers) data.purchasedCovers =[];
            data.purchasedCovers.push(rewardObj.id);
        } else {
            updates.purchasedItems = firebase.firestore.FieldValue.arrayUnion(rewardObj.id);
            if(!data.purchasedItems) data.purchasedItems =[];
            data.purchasedItems.push(rewardObj.id);
        }
    }

    // حفظ التغييرات محلياً أولاً
    localStorage.setItem('currentUser', JSON.stringify(data));

    // 🔥 التحديث الفوري للواجهة بدون ريفرش 🔥
    ...
    if(typeof renderUI === "function") renderUI(data); // تحديث بروفايل الواجهة الرئيسية
    ...
    try {
        ...
        showToast(currentLang === 'en' ? "Error saving reward!" : "حدث خطأ في حفظ الجائزة!");
    }
}

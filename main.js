...
window.runAICoach = function() {
    ...
    if (!coachSection || workouts.length === 0) {
        if(coachSection) coachSection.style.display = 'none';
        return;
    }

    // --- 1. تحليل TSB (الجهد العصبي) ---
    // (منطق مقتبس من دالة initTSBChart للحصول على أحدث TSB)
    let ctl = 0, atl = 0;
    ...
    workouts.forEach(w => {
        ...
        let load = w.details.reduce((sum, ex) => sum + (parseFloat(ex.weight) || 0), 0);
    ...
    // (منطق مقتبس من دالة initMuscleRecoveryMap)
    globalTiredMuscles = {};
    const muscleGroups = { 'صدر': 'chest', 'ظهر': 'back', 'أرجل': 'legs', 'أكتاف': 'shoulders', 'أذرع': 'arms' };
    
    // فحص آخر يومين (48 ساعة)
    workouts.forEach(w => {
        const diffDays = (today - new Date(w.date)) / 86400000;
        if (diffDays <= 2) { 
            const type = getTranslatedType(w.type); 
            for(let key in muscleGroups) {
                if(type.includes(key) || type.includes(muscleGroups[key])) {
                    globalTiredMuscles[key] = true;
                }
            }
        }
    });
    ...
    let message = "";
    let action = "";
    let titleColor = "#00f2a7"; // اللون الافتراضي

    // الحالة 1: إرهاق عصبي شديد (TSB < -20)
    if (currentTSB < -20) {
        message = isEn ? 
            "Critical CNS Fatigue detected. Your nervous system is redlining." : 
            "تحذير: إرهاق عصبي حاد. جهازك العصبي مستنزف بالكامل حالياً.";
        action = isEn ? 
            "Advice: Complete REST day or very light mobility only. No lifting." : 
            "النصيحة: راحة تامة اليوم أو جلسة إطالة خفيفة جداً. ممنوع رفع الأثقال.";
        titleColor = "#ff4d4d";
    } 
    // الحالة 2: جاهزية عالية (TSB > 10)
    else if (currentTSB > 10) {
        const freshMuscles = Object.keys(muscleGroups).filter(m => !globalTiredMuscles[m]);
        message = isEn ? 
            "System Prime! Your CNS is fully recovered and ready for war." : 
            "كابتن، طاقتك في الذروة! جهازك العصبي مستعد لتحطيم الأرقام.";
        action = isEn ? 
            `Target: High Intensity on ${freshMuscles[0] || 'your favorite split'}. Go for a PR!` : 
            `الهدف: تمرين عالي الشدة على (عضلات ${freshMuscles[0] || 'التي تفضلها'}). وقت كسر الـ PR!`;
        titleColor = "#FFD700";
    }
    // الحالة 3: حالة عادية مع عضلات مرهقة (Smart Split)
    else {
        ...
        if (deadMusclesNames.length > 0) {
            message = isEn ? 
                `Muscles (${deadMusclesNames.join(', ')}) are still in repair mode.` : 
                `عضلات (${deadMusclesNames.join(', ')}) لا تزال في مرحلة البناء والترميم.`;
            action = isEn ? 
                `Recommendation: Avoid them today. Focus on ${freshMusclesNames[0] || 'Cardio'}.` : 
                `التوصية: تجنبها اليوم. ركز في تمرينك على ${freshMusclesNames[0] || 'الكارديو'}.`;
        } else {
...
// ==========================================
// 🚨 ربط الدالة بالواجهة (تشغيل المدرب)
// ==========================================
// ابحث عن دالة initDashboardPage، وبالتحديد داخل onAuthStateChanged، بعد سطر renderUI(JSON.parse(localData));
// أضف السطر التالي:
...
function initDashboardPage() {
    auth.onAuthStateChanged(user => {
        if (user) {
            ...
            startBackgroundWarMonitor(user); 
            
            // 🔥 هنا أضفنا استدعاء المدرب الذكي 🔥
            setTimeout(runAICoach, 2000); // تأخير بسيط لضمان تحميل جميع البيانات
...
modal.style.cssText = "position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; background-color: transparent !important; z-index: 999999; display: flex; flex-direction: column; overflow-y: auto; -webkit-tap-highlight-color: transparent; outline: none;";

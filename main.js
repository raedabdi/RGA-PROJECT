...
window.runAICoach = function() {
    ...
    workouts.forEach(w => {
        ...
        if (diffDays <= 2) { 
            // 3. حماية من خطأ الـ TypeError
            const type = (getTranslatedType(w.type) || w.type || "").toLowerCase(); 
            
            // تحليل شامل يغطي الأنظمة (Push/Pull/Full)
    ...
    if (currentTSB < -20) {
        ...
        const freshMuscles = Object.keys(muscleGroups).filter(m => !globalTiredMuscles[m]);
        ...
        action = isEn ? `Target: High Intensity on ${freshMuscles[0] || 'your favorite split'}. Go for a PR!` : `الهدف: تمرين عالي الشدة (يُفضل ${freshMuscles[0] || 'عضلتك المفضلة'}). وقت كسر الـ PR!`;
        titleColor = "#FFD700";
    } else {
        ...
        if (deadMusclesNames.length > 0) {
            message = isEn ? `Muscles (${deadMusclesNames.join(', ')}) are still in repair mode.` : `عضلات (${deadMusclesNames.join('، ')}) لا تزال في مرحلة الاستشفاء.`;
            action = isEn ? `Recommendation: Avoid them today. Focus on ${freshMusclesNames[0] || 'Cardio'}.` : `التوصية: تجنب إرهاقها اليوم. ركز في تمرينك على (${freshMusclesNames[0] || 'الكارديو'}).`;
        } else {
...
// ==========================================
// 🎮 محرك لعبة صائد الأوزان (Iron Reflex - Emotional Symphony المصحح)
// ==========================================

let audioCtx = null;

// السلالم الموسيقية لكل حالة شعورية
const emotionalScales = {
    deep: [130.81, 146.83, 164.81, 196.00], 
    sad: [196.00, 220.00, 233.08, 261.63, 293.66], 
    melancholic: [261.63, 311.13, 349.23, 392.00, 466.16], 
    hopeful: [261.63, 293.66, 329.63, 392.00, 440.00], 
    happy: [392.00, 440.00, 493.88, 523.25, 587.33], 
    energetic: [440.00, 493.88, 554.37, 587.33, 659.25], 
    heroic: [261.63, 329.63, 392.00, 523.25, 659.25], 
    cyber: [130.81, 261.63, 523.25, 1046.50], 
    aggressive: [82.41, 110.00, 123.47, 164.81], 
    insane: [440.0, 466.16, 880.0, 932.33], 
    void: [50, 100, 200, 400, 800], 
    hell: [40, 45, 50, 55]
};

let currentMood = 'deep';
let moodNoteCounter = 0;

// دالة تشغيل النغمة الموسيقية (مصححة ومضمونة)
const playEmotionalNote = (combo) => {
    try {
        ...
        // 1. تحديد المود بناءً على الكومبو
        if (combo >= 600) currentMood = 'void';
        else if (combo >= 500) currentMood = 'hell';
        else if (combo >= 400) currentMood = 'aggressive';
        else if (combo >= 300) currentMood = 'energetic';
        else if (combo >= 200) currentMood = 'happy';
        else if (combo >= 100) currentMood = 'hopeful';
        else if (combo >= 50) currentMood = 'sad';
        else currentMood = 'deep';

        // 2. تغيير نوع الموجة (الآلة) بناءً على المود
        if (['aggressive', 'hell', 'insane'].includes(currentMood)) osc.type = 'sawtooth';
        else if (['cyber', 'void'].includes(currentMood)) osc.type = 'square';
        else if (['happy', 'energetic'].includes(currentMood)) osc.type = 'triangle';
        else osc.type = 'sine';

        // 3. جلب النغمة الصحيحة
        const currentScale = emotionalScales[currentMood] || emotionalScales.deep;
        let note = currentScale[moodNoteCounter % currentScale.length];
        
        // ذكاء اصطناعي بسيط لتنويع اللحن
        if (Math.random() > 0.8) note = currentScale[Math.floor(Math.random() * currentScale.length)];
        moodNoteCounter++;
        ...
        if (board) board.className = 'reflex-board-container mood-' + currentMood;
    } catch (e) {
...
window.openReflexGame = function() {
    ...
    document.getElementById('reflex-board').className = 'reflex-board-container mood-deep';
...
// بدء اللعب (عند الضغط على Tap to Start)
...
function spawnReflexTile() {
    ...
    const isGold = Math.random() < 0.02; // 2% للقلب
...
window.handleReflexTap = function(laneIndex, event) {
    ...
    // البحث عن أقرب وزن في نفس المسار
    ...
    if (targetTile && tapY >= targetTile.y - 25 && tapY <= targetTile.y + TILE_HEIGHT + 25) {
        ...
        playEmotionalNote(reflexCombo); // تشغيل الصوت والمود
        
        if(navigator.vibrate) navigator.vibrate(15);
        if (targetTile.isGold && reflexLives < 3) { reflexLives++; updateLivesUI(); }

        // تأثير الوميض والحذف
        const tileEl = targetTile.element;
        tileEl.classList.add('tile-hit');
        setTimeout(() => { if(tileEl) tileEl.remove(); }, 100);

        reflexTilesData = reflexTilesData.filter(t => t.id !== targetTile.id);
        reflexScore++;
        
        // زيادة السرعة وتحديث العدادات
        reflexSpeed += 0.04; 
        document.getElementById('reflex-score-val').innerText = reflexScore;
        const speedMultiplier = (reflexSpeed / 4.5).toFixed(1);
        document.getElementById('reflex-speed-val').innerText = 'x' + speedMultiplier;
...
function endReflexGame(userQuit) {
    ...
    setTimeout(() => {
        ...
        let earnedXP = Math.min(Math.floor(reflexScore / 2), 75);
        if (earnedXP > 0) {

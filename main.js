...
async function viewPlayerProfile(targetUid) {
    ...
    try {
        ...
        if (myUid && myUid !== targetUid) {
            ...
            if (isFriend) {
                const friendBtnText = lang === 'en' ? '<i class="fa-solid fa-user-check"></i> Friend 🤝' : 'صديق 🤝 <i class="fa-solid fa-user-check"></i>';
                ...
                friendActionHTML = `
                    <div style="display: flex; gap: 10px; margin-top: 15px; width: 100%;">
                        <button class="btn-primary" style="flex: 1; background: rgba(0, 242, 167, 0.1); border-color: #00f2a7; color: #00f2a7; cursor: default; display: flex; align-items: center; justify-content: center; gap: 8px;">
                            ${friendBtnText}
                        </button>
                        <div style="position: relative; display: flex;">
                            
                            <button class="member-menu-trigger-btn" onclick="toggleMemberMenu('profile-friend-menu', this)">
                                <i class="fa-solid fa-ellipsis-vertical"></i>
                            </button>
                            
                            <div id="profile-friend-menu" class="member-action-menu-content" style="background: rgba(10, 20, 41, 0.95); border: 1px solid rgba(255, 77, 77, 0.3); border-radius: 8px; padding: 4px; box-shadow: 0px 5px 20px rgba(0,0,0,0.8); min-width: 140px;">
                                <div class="dropdown-item-pro danger" onclick="deleteFriendFromProfile('${targetUid}')" style="padding: 10px; font-size: 0.9rem; display: flex; align-items: center; justify-content: center; gap: 10px; color: #ff4d4d; cursor: pointer; border-radius: 5px; white-space: nowrap; margin: 0;">
                                    ${unfriendBtnText}
                                </div>
                            </div>
                        </div>
                ...
                if (reqDoc.exists) {
                    ...
                    friendActionHTML = `
                        <button class="btn-primary" style="width: 100%; margin-top: 15px; background: rgba(255,255,255,0.05); border-color: var(--slate); color: var(--slate); cursor: not-allowed; display: flex; align-items: center; justify-content: center; gap: 8px;" disabled>
                            ${reqSentText}
                        </button>
                    `;
                } else {
                    ...
                    friendActionHTML = `
                        <button class="btn-primary" id="profile-add-friend-btn" style="width: 100%; margin-top: 15px; box-shadow: 0 0 15px rgba(0,242,167,0.2); display: flex; align-items: center; justify-content: center; gap: 8px;" onclick="sendFriendRequestFromProfile('${targetUid}')">
                            ${addFriendText}
                        </button>
                    `;
                }
        ...
        if (data.clanId) {
            ...
            clanInfoHTML = `
                <div class="premium-clan-badge" onclick="closeProfileAndOpenClan('${data.clanId}')" style="margin-top:10px;">
            ...
            db.collection('clans').doc(data.clanId).get().then(doc => {
                ...
                else if (nameEl) nameEl.innerText = lang === 'en' ? 'Unknown Clan' : 'عصابة غير معروفة';
            }).catch(e => console.log(e));
        } else if (myUserData.clanId) {
            const txtInvite = lang === 'en' ? 'Invite to Clan' : 'دعوة للعصابة';
            clanInfoHTML = `
                <button class="btn-primary" style="width:100%; margin-top:10px; background: rgba(0, 242, 167, 0.1); border: 1px solid var(--primary-color); color:var(--primary-color);" onclick="sendClanInvite('${targetUid}')">
...
window.sendClanInvite = async function(targetUid) {
    ...
    try {
        // نصوص الإشعار المترجمة
        const notifTitle = lang === 'en' ? "Clan Invitation 🛡️" : "دعوة لعصابة حديد 🛡️";
        const notifBody = lang === 'en' 
            ? `Captain ${myData.firstName || 'Hero'} invited you to join their clan!` 
            : `لقد دعاك الكابتن ${myData.firstName || 'بطل'} للانضمام إلى عصابته!`;

        await db.collection('users').doc(targetUid).collection('notifications').add({
            title: notifTitle,
            text: notifBody, // غيرناها لـ text لتتوافق مع نظام إشعاراتك
            type: "clan_invite",
        ...
        const txtSuccess = lang === 'en' ? 'Invitation sent successfully!' : 'تم إرسال الدعوة بنجاح!';
        showToast(txtSuccess);

    } catch (err) {
        console.error(err);
        const txtError = lang === 'en' ? 'Error sending invitation.' : 'حدث خطأ في إرسال الدعوة';
        showToast(txtError);
    }
};
...
window.deleteFriendFromProfile = async function(friendId) {
    ...
    const lang = currentLang || 'ar';
    const confirmMsg = lang === 'en' ? "Remove this hero from your friends?" : "متأكد إنك بدك تحذف هالبطل من أصدقائك؟";
    ...
    if (confirm(confirmMsg)) {
        try {
            ...
            // تحديث البيانات المحلية لتتحدث الشاشة فوراً (للمستخدم الحالي فقط)
            let localData = JSON.parse(localStorage.getItem('currentUser') || '{}');
            localData.myFriendsList = updatedMyFriends;
            localStorage.setItem('currentUser', JSON.stringify(localData));

            // 2. تحديث حساب صديقك: إزالتك أنت من قائمته (الحذف المتبادل) في قاعدة البيانات فقط
            const friendDoc = await db.collection('users').doc(friendId).get();
            if (friendDoc.exists) {
                // ✅ التصحيح هنا: إضافة [] لتجنب الخطأ
                let theirFriends = friendDoc.data()?.myFriendsList || [];
                let updatedTheirFriends = theirFriends.filter(f => f.id !== currentUser.uid);
                await db.collection('users').doc(friendId).update({ myFriendsList: updatedTheirFriends });
            }

...
window.acceptFriendRequest = async function(notifId, senderId, senderName, senderPhoto) {
    ...
    try {
        ...
        // 3. 🟢 التعديل السحري: إضافة الصديق فوراً للذاكرة المحلية ليظهر بدون ريفرش
        let localData = JSON.parse(localStorage.getItem('currentUser') || '{}');
        if (!localData.myFriendsList) localData.myFriendsList = [];
        
        // التأكد أنه غير موجود مسبقاً لتجنب التكرار
        if (!localData.myFriendsList.find(f => f.id === senderId)) {
            localData.myFriendsList.push({ id: senderId, name: senderName, photoURL: senderPhoto });
            localStorage.setItem('currentUser', JSON.stringify(localData));
        }

        // 4. تحديث قائمة الأصدقاء فوراً في الخلفية
        if (typeof renderMyFriends === 'function') {
            renderMyFriends();
        }

        // 5. تقليل رقم عداد الإشعارات الأحمر (الجرس)
...
window.deleteFriendFromProfile = async function(friendId) {
    const currentUser = auth.currentUser; 
    if (!currentUser) return;
    
    const lang = currentLang || 'ar';
    
    // استخدام SweetAlert2 لتأكيد فخم يناسب RGA Fit
    Swal.fire({
        title: lang === 'en' ? 'Are you sure?' : 'هل أنت متأكد؟',
        text: lang === 'en' ? "Remove this hero from your friends?" : "متأكد إنك بدك تحذف هالبطل من أصدقائك؟",
        icon: 'warning',
        showCancelButton: true,
        background: '#0A1429',
        color: '#ffffff',
        confirmButtonColor: '#ff4d4d',
        cancelButtonColor: '#00f2a7',
        confirmButtonText: lang === 'en' ? 'Yes, remove' : 'نعم، احذف',
        cancelButtonText: lang === 'en' ? 'Cancel' : 'إلغاء'
    }).then(async (result) => {
        if (result.isConfirmed) {
            try {
                // 1. تحديث حسابك أنت: إزالة الصديق من قائمتك
                const myDoc = await db.collection('users').doc(currentUser.uid).get();
                let myFriends = myDoc.data()?.myFriendsList || [];
                let updatedMyFriends = myFriends.filter(f => f.id !== friendId);
                
                await db.collection('users').doc(currentUser.uid).update({ 
                    myFriendsList: updatedMyFriends 
                });

                // تحديث البيانات المحلية 
                let localData = JSON.parse(localStorage.getItem('currentUser') || '{}');
                localData.myFriendsList = updatedMyFriends;
                localStorage.setItem('currentUser', JSON.stringify(localData));

                // 2. تحديث حساب صديقك: إزالتك أنت من قائمته
                const friendDoc = await db.collection('users').doc(friendId).get();
                if (friendDoc.exists) {
                    let theirFriends = friendDoc.data()?.myFriendsList || [];
                    let updatedTheirFriends = theirFriends.filter(f => f.id !== currentUser.uid);
                    await db.collection('users').doc(friendId).update({ myFriendsList: updatedTheirFriends });
                }
                
                // 3. 🟢 السحر هنا: تغيير شكل الزر في البروفايل فوراً بدون ريفرش
                const menuContainer = document.querySelector('.member-actions-menu');
                if (menuContainer) {
                    // إزالة قائمة الثلاث نقاط بالكامل
                    menuContainer.remove();
                    
                    // تحويل زر "صديق" الأخضر إلى زر "إرسال طلب"
                    const friendStatusBtn = document.querySelector('.fa-user-check').parentElement;
                    if (friendStatusBtn) {
                        friendStatusBtn.outerHTML = `
                            <button class="btn-primary" id="profile-add-friend-btn" style="width: 100%; margin-top: 15px; box-shadow: 0 0 15px rgba(0,242,167,0.2);" onclick="sendFriendRequestFromProfile('${friendId}')">
                                <i class="fa-solid fa-user-plus"></i> ${lang === 'en' ? 'Send Friend Request' : 'إرسال طلب صداقة'}
                            </button>
                        `;
                    }
                }
                
                // 4. تحديث قائمة الأصدقاء في الخلفية إذا كانت مفتوحة
                if (typeof renderMyFriends === 'function') renderMyFriends();
                
                showToast(lang === 'en' ? "Friend removed" : "تم حذف الصديق");
...
window.openClanFromInvite = async function(notifId, clanId) {
    // 1. إغلاق قائمة الإشعارات فوراً
    const dropdown = document.getElementById('notif-dropdown');
    if (dropdown) dropdown.classList.remove('show');

    // 2. استدعاء دالة فتح البروفايل الخاص بالكلان (الموجودة مسبقاً في كودك)
    if (typeof closeProfileAndOpenClan === 'function') {
        closeProfileAndOpenClan(clanId);
    } else {
        // إذا لم تكن الدالة موجودة، نقوم بفتح الكلان يدوياً
        openGuildHub(); // نفتح شاشة الكلان الرئيسية
        setTimeout(() => {
            renderClanPreview(null, clanId, translations[currentLang || 'ar']);
        }, 500);
    }
    
    // 3. (اختياري) مسح الإشعار بعد النقر عليه أو تركه حسب رغبتك
...
window.closeProfileAndOpenClan = async function(clanId) {
    // 1. إغلاق واجهات البروفايل المفتوحة
    const modals = document.querySelectorAll('.modal, [id*="profile-modal"], [id*="player-profile"]');
    ...
    showToast(lang === 'en' ? "Loading Clan..." : "جاري استطلاع العصابة...");

    try {
        // 2. جلب بيانات الكلان أولاً لمنع الانهيار (Crash)
        const clanDoc = await db.collection('clans').doc(clanId).get();
        if (!clanDoc.exists) {
            showToast(lang === 'en' ? "Clan not found!" : "هذه العصابة لم تعد موجودة!");
            return;
        }

        const clanData = clanDoc.data();

        // 3. بناء مساحة الشاشة فوراً (لأننا في صفحة أخرى ولا توجد مساحة للكلان)
        const mainContent = document.getElementById('main-content-area');
        if (mainContent) {
            mainContent.innerHTML = `
                <header class="top-bar" style="margin-bottom: 20px;">
                    <div class="header-row" style="display:flex; justify-content:space-between; width:100%; align-items: center;">
                        <button onclick="backToDashboard()" class="btn-primary" style="padding: 5px 15px;">${t.back || 'رجوع'}</button>
                        <h1 style="margin: 0; font-weight: 900; color: #FFD700; text-shadow: 0 0 10px rgba(255,215,0,0.3);">
                            <i class="fa-solid fa-magnifying-glass"></i> ${lang === 'en' ? 'Clan Preview' : 'استطلاع العصابة'}
                        </h1>
            ...
            // 4. تشغيل دالة استطلاع الكلان وتمرير البيانات الحقيقية لها 100%
            if (typeof renderClanPreview === 'function') {
                renderClanPreview(clanData, clanId, t);
            } else {
                console.error("دالة renderClanPreview مفقودة!");
            }
        }
    } catch (e) {
        console.error("Error opening clan:", e);
        showToast(lang === 'en' ? "Error loading clan!" : "حدث خطأ في تحميل بيانات العصابة!");
    }
};
